...
$oi=$_oi->GetItemById($oif['user_d_id']);
if($oi===false){
	header("HTTP/1.1 404 Not Found");
	header("Status: 404 Not Found");
	include("404.php");
...
$user=$_user->GetItemById($oif['user_d_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(1, $oif['user_d_id'], new PetitionFileItem(1));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл заявления',NULL,725, NULL,SecStr('файл '.$oif['orig_name'].' для '.  $user['code'].' '.$user['name'].$_folder_desrc),$user['id']);
