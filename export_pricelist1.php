...
require_once('classes/pl_posgroup_excel1.php');
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('A1', iconv('windows-1251', 'utf-8','Наименование оборудования '))
			->setCellValue('B1', iconv('windows-1251', 'utf-8','Цена ExW пост-ка'))
        
			->setCellValue('C1', iconv('windows-1251', 'utf-8','Цена ExW'))
			->setCellValue('D1', iconv('windows-1251', 'utf-8','Рент-ть ExW, %'))
			
			->setCellValue('E1', iconv('windows-1251', 'utf-8','Цена DDPM'))
			->setCellValue('F1', iconv('windows-1251', 'utf-8','Рент-ть DDPM, %'))
...
foreach($alls as $k=>$v){
	...
	foreach($v['cats'] as $k1=>$v1){
		...
		foreach($v1['goods'] as $k2=>$v2){
			...
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $index,  $good);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $index,  (float)$v2['price_base_exw']);
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $index,  (float)$v2['price_exw']);
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $index,  (float)$v2['profit_exw']);
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $index,  (float)$v2['price_ddpm']);
			
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $index,  (float)$v2['profit_ddpm']);
...
$objPHPExcel->getActiveSheet()->getStyle('A1:F'.($index-1).'')->applyFromArray($styleArray);
