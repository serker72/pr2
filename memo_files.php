...
$smarty->assign("SITETITLE",'Файлы служебной записки');
...
if(!isset($_GET['bill_id'])){
	if(!isset($_POST['bill_id'])){
		...
		include("404.php");
		die();		
	}else $petition_id=abs((int)$_POST['bill_id']);
	
}else $petition_id=abs((int)$_GET['bill_id']); 
...
if(!$au->user_rights->CheckAccess('w',731)){ //705)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if($folder_id==0){
	...
	$_ff=new FileDocFolderItem(1, $petition_id, new MemoFileItem(1));
...
$log->PutEntry($result['id'],'открыл реестр файлов служебной записки',NULL,731, NULL, $user['code'].$_folder_desrc,$petition_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	if(!$au->user_rights->CheckAccess('w',1133)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	...
	$_file=new MemoFileItem;
	$file=$_file->GetItemById($id);
	
	if($file!==false){
		$_file->Del($id);
		
		$log->PutEntry($result['id'],'удалил файл',NULL,1133,NULL,'имя файла '.SecStr($file['orig_name']),$kp_id);
	}
	
	header("Location: memo_files.php?bill_id=".$petition_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,1133);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,1133,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
	
	header("Location: memo_files.php?bill_id=".$petition_id.'&folder_id='.$folder_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	$_ff->SetDocIdName('bill_id');
		    ...
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 1133);
...
$_menu_id=59;
	...
	$decorator->AddEntry(new UriEntry('bill_id',$petition_id));
	
	$ffg=new MemoFileGroup(1, $petition_id,  new FileDocFolderItem(1,  $petition_id, new MemoFileItem(1)));
	...
	$sm->assign('bill',$user);
	$content=$sm->fetch('memo/files.html');
