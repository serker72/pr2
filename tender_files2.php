...
$smarty->assign("SITETITLE",'Файлы тендера');
...
$log->PutEntry($result['id'],'открыл реестр файлов тендера',NULL,931, NULL, $user['code'].$_folder_desrc,$bill_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил файл',NULL,931,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
	}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил папку',NULL,931,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
