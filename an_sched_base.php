...
//******************************************************************************************************************	 
	...
	$filetext=$as1->ShowData($_suppliers, $_users,  $viewed_ids,  $pdate1, $pdate2, 'an_sched/an_sched'.$prefix.$print_add.'.html',$decorator,'an_sched.php',   isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x'])/*||($print==1)*/,  $au->user_rights->CheckAccess('w',903),  $au->user_rights->CheckAccess('w',905), $alls, $result, $supplier_kinds);
...
//******************************************************************************************************************	 
	...
	elseif(isset($_GET['doSub7'])||isset($_GET['doSub7_x'])){
		...
		//$comand = "wkhtmltopdf-i386  --load-error-handling skip --encoding windows-1251 --page-size A4 ".$orient." --margin-top 73mm --margin-bottom 27mm --margin-left 10mm --margin-right 10mm --footer-html ".SITEURL."/tmp/".$ftmp.".html --header-html ".SITEURL."/tpl-sm/plan_pdf/pdf_header.html  ".SITEURL.'/tmp/'.$tmp.'.html'."  ".ABSPATH.'/tmp/'."$tmp.pdf 1>".ABSPATH.'/tmp/'."data 2> ".ABSPATH.'/tmp/'."exit";
