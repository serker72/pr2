...
$smarty->assign("SITETITLE",'Файлы заявки на договор');
...
if(!$au->user_rights->CheckAccess('w',1150)){ //705)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
$log->PutEntry($result['id'],'открыл реестр файлов заявки на договор',NULL,1150, NULL, $user['code'].$_folder_desrc,$bill_id);	
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	$_file=new AppContractFileItem;
	$file=$_file->GetItemById($id);
	
	if($file!==false){
		$_file->Del($id);
		
		$log->PutEntry($result['id'],'удалил файл',NULL,1150,NULL,'имя файла '.SecStr($file['orig_name']),$bill_id);
	}
	
	header("Location: app_contract_files.php?bill_id=".$bill_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,1150);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,1150,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
	
	header("Location:app_contract_files.php?bill_id=".$bill_id.'&folder_id='.$folder_id);
	die();
}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	if(is_array($files)&&(count($files)>0)){
		foreach($files as $v){	
		  ...
		  $_file=new AppContractFileItem(2);
	...
	$_ff->SetDocIdName('doc_id');
		    ...
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 1150);
...
//$_menu_id=NULL;
	...
	$sm->assign('bill',$user);
	$content=$sm->fetch('app_contract/files.html');
