...
if($action==1){
	$log=new ActionLog;
	$log->PutEntry($result['id'],'открыл карту виртуального сотрудника',NULL,772, NULL, $editing_user['login'].' '.$editing_user['name_s'],$id);			
}
...
//	$params['position_s']=SecStr($_POST['position_s']);
	...
	if($code>0){
		...
		foreach($params as $k=>$v){
				...
				$log->PutEntry($result['id'],'создал пользователя V',NULL,771, NULL, 'в поле '.$k.' установлено значение '.$v,$code);		
	...
	foreach($params as $k=>$v){
		...
		if(addslashes($editing_user[$k])!=$v){
			...
			if($k=='position_id'){
				...
				$log->PutEntry($result['id'],'редактировал пользователя V',$id,772, NULL, 'установлена должность '.SecStr($up['name']),$id);	
				continue;	
			}
			...
			if($k=='department_id'){
				$_up=new UserDepItem;
				$up=$_up->getitembyid($v);
				$log->PutEntry($result['id'],'редактировал пользователя V',$id,772, NULL, 'установлен отдел '.SecStr($up['name']),$id);	
				continue;	
			}
...
$_menu_id=8;
	...
	//	var_dump($up);
		...
		$_usg=new UsersSGroup;
		$dec1=new DbDecorator;
		$dec1->AddEntry(new SqlEntry('u.id',$editing_user['id'], SqlEntry::NE));
		$dec1->AddEntry(new SqlOrdEntry('name_s',SqlOrdEntry::ASC));
		$sm1->assign('managers', $_usg->GetItemsByDecOpt($editing_user['manager_id'],'name_s',$dec1,true));
		
		//подчиненные
		$dec1=new DbDecorator;
		$dec1->AddEntry(new SqlEntry('u.manager_id',$editing_user['id'], SqlEntry::E));
		$dec1->AddEntry(new SqlOrdEntry('name_s',SqlOrdEntry::ASC));
		...
		foreach($pod as $k=>$v){
			...
			$pod1=$_usg->GetItemsByDecArr($dec1);	
			 foreach($pod1 as $k1=>$v1) $pod[]=$v1;
