...
if($fmi['folder_id']==0){
	...
	$_ff=new FileDocFolderItem(4, $fmi['kp_id'], new KpFileItem(4));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл коммерческого предложени€',NULL,695, NULL,SecStr('файл '.$fmi['orig_name'].' дл€ '.  $user['code'].' '.$user['name'].$_folder_desrc),$user['id']);
