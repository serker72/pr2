...
class InvPrepareGroup extends AbstractGroup {
	...
	public function GetItemsByDateStorSec($pdate, $org_id,$is_id=0, DBDecorator $dec=NULL){
		$alls=array();
		...
		$sql2='select sum(ap.quantity) as quantity_as_is, 
				ap.position_id as id, ap.position_id as position_id, ap.pl_position_id as pl_position_id, ap.name as position_name, ap.dimension as dim_name, dim.id as dimension_id 
		...
		for($i=0; $i<$rc; $i++){
			...
			*/
			...
			if($f['quantity_as_is']<=0) continue;
