...
class AnSchedSu{
	...
	public function ShowData( $supplier_ids,  $user_ids,  $viewed_ids_arr, $pdate1, $pdate2,  $template, DBDecorator $dec2,$pagename='files.php',  $do_it=false, $can_print=false, $can_edit=false, &$alls, $result=NULL, $supplier_kinds=NULL, $sortmode=NULL, $is_fulfil=NULL){
		...
		us.name_s as confirmed_shipping_name, us.login as confirmed_shipping_login, p.confirm_done_pdate as confirm_shipping_pdate,
			...
			where  ss.holding_id in(  '.implode(', ',$supplier_ids).')  )';
				...
				$sql.='select distinct p.id,  6 as kind_id,  p.kind_id as kkind_id, 6 as document_type_id,
			p.code, p.pdate_claiming as pdate_beg, 	"" as pdate_end, "" as ptime_beg, "" as ptime_end, p.pdate, "" as plan_or_fact, "" as incoming_or_outcoming, "" as priority, p.topic as topic, "" as report,
		...
		cr.name_s as cr_name, cr.login as cr_login, cr.is_active as cr_is_active 
				...
				where 
				 ...
				 p.pdate_claiming between "'.date('Y-m-d', DateFromdmY($pdate1)).'" and "'.date('Y-m-d', DateFromdmY($pdate2)).'" 
				...
				if(count($user_ids)>0){
					$sort_flt='';
					$sort_flt='
					and  (p.manager_id in('.implode(', ',$user_ids).') or p.created_id in('.implode(', ',$user_ids).'))					
					';
				...
				if(count($supplier_ids)>0){
					...
					$_for_nn=array();
					$_for_nn[]='p.id in(select distinct sched_id from  tender_suppliers where supplier_id in ('.implode(', ',$supplier_ids).')) ';
					if($has_holdings){
						//1. записи по тем контрагентам, у кого холдинг=заданному к-ту
						$_for_nn[]='p.id in(select distinct ps.sched_id from  tender_suppliers as ps inner join supplier as ss on ss.id=ps.supplier_id and ss.is_active=1 and ss.holding_id in( '.implode(', ',$supplier_ids).'))';
						 
						//2. найти все субхолдинги заданного к-та (у кого он холдинг, связь через контрагентов)
						$_for_nn[]='p.id in(select distinct ps.sched_id from  tender_suppliers as ps inner join supplier as ss on ss.id=ps.supplier_id where ss.is_active=1 and ss.id in(select distinct subholding_id from supplier where is_active=1 and holding_id in(  '.implode(', ',$supplier_ids).')))';
						 
						//3. найти все дочерние предприятия субхолдингов заданного предприятия
						$_for_nn[]='p.id in(select distinct ps.sched_id from  tender_suppliers as ps 
			inner join supplier as ss on ss.id=ps.supplier_id and ss.is_active=1  /*запись контрагента, у кого холдинг и субхолдинг определены */
			inner join supplier as sub on sub.id=ss.subholding_id and sub.is_active=1  /*запись субхолдинга*/
			inner join supplier as doch on sub.id=doch.subholding_id and doch.is_active=1  /*дочерняя компания субхолдинга */
			where  ss.holding_id in(  '.implode(', ',$supplier_ids).')  )';
						...
						//4. найти всех контрагентов, у кого субхолдинг = заданному
						$_for_nn[]='p.id in(select distinct ps.sched_id from  tender_suppliers as ps inner join supplier as ss on ss.id=ps.supplier_id and ss.is_active=1 and ss.subholding_id in( '.implode(', ',$supplier_ids).'))';
				...
				if(($supplier_kinds!==NULL)&&is_array($supplier_kinds)){
					...
					$su_flt='';	
					$su_flt='
					and p.id in(select distinct sched_id from  tender_suppliers where supplier_id in (select id from supplier where '.implode(' OR ',$_flt).')) 
				...
				//фильтр по видимости
				if(count($viewed_ids_arr[6])>0){
					
					$su_flt='';	
					$su_flt='
					and 
					  (p.manager_id in('.implode(', ',$viewed_ids_arr[6]).') or p.created_id in('.implode(', ',$viewed_ids_arr[6]).')) 
				...
				$sql.='select distinct p.id,  7 as kind_id, p.kind_id as kkind_id, 7 as document_type_id,
			p.code, p.pdate_finish as pdate_beg, 	"" as pdate_end, "" as ptime_beg, "" as ptime_end, p.pdate, "" as plan_or_fact, "" as incoming_or_outcoming, "" as priority, p.topic as topic, "" as report,
		...
		cr.name_s as cr_name, cr.login as cr_login, cr.is_active as cr_is_active 
				...
				where 
				 ...
				 p.pdate_finish between "'.date('Y-m-d', DateFromdmY($pdate1)).'" and "'.date('Y-m-d', DateFromdmY($pdate2)).'" 
				...
				if(count($supplier_ids)>0){
					$su_flt='';	
					/*$su_flt='
					and  p.id in(select distinct sched_id from  lead_suppliers where supplier_id in ('.implode(', ',$supplier_ids).')) 
					 
					';*/
					$_for_nn=array();
					$_for_nn[]='p.id in(select distinct sched_id from  lead_suppliers where supplier_id in ('.implode(', ',$supplier_ids).')) ';
					if($has_holdings){
						//1. записи по тем контрагентам, у кого холдинг=заданному к-ту
						$_for_nn[]='p.id in(select distinct ps.sched_id from  lead_suppliers as ps inner join supplier as ss on ss.id=ps.supplier_id and ss.is_active=1 and ss.holding_id in( '.implode(', ',$supplier_ids).'))';
						 
						//2. найти все субхолдинги заданного к-та (у кого он холдинг, связь через контрагентов)
						$_for_nn[]='p.id in(select distinct ps.sched_id from  lead_suppliers as ps inner join supplier as ss on ss.id=ps.supplier_id where ss.is_active=1 and ss.id in(select distinct subholding_id from supplier where is_active=1 and holding_id in(  '.implode(', ',$supplier_ids).')))';
						 
						//3. найти все дочерние предприятия субхолдингов заданного предприятия
						$_for_nn[]='p.id in(select distinct ps.sched_id from  lead_suppliers as ps 
			inner join supplier as ss on ss.id=ps.supplier_id and ss.is_active=1  /*запись контрагента, у кого холдинг и субхолдинг определены */
			inner join supplier as sub on sub.id=ss.subholding_id and sub.is_active=1  /*запись субхолдинга*/
			inner join supplier as doch on sub.id=doch.subholding_id and doch.is_active=1  /*дочерняя компания субхолдинга */
			where  ss.holding_id in(  '.implode(', ',$supplier_ids).')  )';
						...
						//4. найти всех контрагентов, у кого субхолдинг = заданному
						$_for_nn[]='p.id in(select distinct ps.sched_id from  lead_suppliers as ps inner join supplier as ss on ss.id=ps.supplier_id and ss.is_active=1 and ss.subholding_id in( '.implode(', ',$supplier_ids).'))';
				...
				if(($supplier_kinds!==NULL)&&is_array($supplier_kinds)){
					...
					$su_flt='';	
					$su_flt='
					and p.id in(select distinct sched_id from  lead_suppliers where supplier_id in (select id from supplier where '.implode(' OR ',$_flt).')) 
				...
				//фильтр по видимости
				if(count($viewed_ids_arr[7])>0){
					
					$su_flt='';	
					$su_flt='
					and 
					  (p.manager_id in('.implode(', ',$viewed_ids_arr[7]).') or p.created_id in('.implode(', ',$viewed_ids_arr[7]).')) 
				...
				for($i=0; $i<$rc; $i++){
					...
					if($f['kind_id']==6){
						$f['suppliers']=$_tsg->GetItemsByIdArr($f['id']);
					...
					if($f['kind_id']==7){
						$f['suppliers']=$_lsg->GetItemsByIdArr($f['id']);
