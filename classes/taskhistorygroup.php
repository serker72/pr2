...
// абстрактная группа
class TaskHistoryGroup extends AbstractGroup {
	...
	public function CountNew($order_id,$user_id){
		$ts=new mysqlSet('select * from task where id="'.$order_id.'"');
		$trs=$ts->GetResult();
	...
	public function CountNewOrders($user_id){
		...
		$man=new DiscrMan;
		if($man->CheckAccess($user_id,'w',589)){
			$ts=new mysqlSet('select count(distinct id) from task where id in(select distinct task_id from task_history where user_id<>"'.$user_id.'" and is_new=1)');
			
		}else{
			 $ts=new mysqlSet('select count(distinct id) from task where (user_id="'.$user_id.'" OR user_id="0") and id in(select distinct task_id from task_history where user_id<>"'.$user_id.'" and is_new=1) /*and status_id<>4*/');
