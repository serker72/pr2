...
class BillReports extends AbstractGroup{
	...
	*/
	...
	/*public function InAcc($kvid,$template,$org_id,$is_ajax=true){
		if($is_ajax) $sm=new SmartyAj;
		else $sm=new SmartyAdm;
				
		$sql='select distinct s.id, s.pdate, s.bill_id, b.code, b.pdate as bill_pdate, bp.quantity   from acceptance as s inner join acceptance_position as bp on s.id=bp.acceptance_id left join bill as b on s.bill_id=b.id where bp.komplekt_ved_pos_id="'.$kvid.'" and s.is_confirmed=1  and s.org_id="'.$org_id.'"';
