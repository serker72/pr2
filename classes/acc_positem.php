...
class AccPosItem extends AbstractItem{
	...
	public function SetChainQuantity($acceptance_id,$position_id, $pl_position_id, $pl_discount_id, $pl_discount_value, $pl_discount_rub_or_percent, $quantity,$change_high_mode=0,$change_low_mode=0, $_result=NULL, $item=NULL, $out_bill_id=0, $kp_id=0){
		...
		if($ai!==false){
			...
			if($bpi!==false){
				...
				(($delta_rasp<0)&&($change_low_mode==1))*/
					...
					$note='Количество позиции '.SecStr($bpi['name']).' автоматически выровнено при утверждении связанной реализации №'.$acceptance_id.' сотрудником '.$_result['name_s'].' ('.$_result['login'].')'.'. Первичное количество '.$bpi['quantity'].', конечное количество '.$new_quantity;
					...
					$log->PutEntry($_result['id'],'автоматическое выравнивание позиции распоряжения на отгрузку при утверждении связанной реализации',  NULL, 217,NULL, $note, $ai['sh_i_id']);
					
					
					//автопримечание в поступление
					$_ani=new AccNotesItem;
					$note='Количество позиции '.SecStr($bpi['name']).' в распоряжении на отгрузку №'.$ai['sh_i_id'].' автоматически выровнено при утверждении реализации сотрудником '.$_result['name_s'].' ('.$_result['login'].')'.'. Первичное количество '.$bpi['quantity'].', конечное количество '.$new_quantity;
