...
class TrustPrepare extends BillPosGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0){
		...
		$sql='select sum(p.quantity) as quantity,  p.bill_id,  p.position_id as id, p.position_id as position_id, p.pl_position_id as pl_position_id,
		...
		group by p.position_id, p.pl_position_id
		order by p.name asc
		';
