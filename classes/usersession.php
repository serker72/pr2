...
class UserSession extends AbstractItem{
	...
	public function GetUsersOnline($usermode){
		$arr=array();
				...
				$sql='select distinct us.id, u.name_s, up.name as position,  u.login, u.group_id, g.name  from user_session as us inner join user as u on us.user_id=u.id left join groups as g on u.group_id=g.id left join user_position as up on up.id=u.position_id where u.id<>1 and u.id<>90 order by  u.login asc, u.group_id asc';
