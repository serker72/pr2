...
// абстрактная группа
class MemoHistoryGroup extends AbstractGroup {
	...
	public function CountNew($order_id,$user_id){
		$ts=new mysqlSet('select * from memo where id="'.$order_id.'"');
		$trs=$ts->GetResult();
	...
	public function CountNewOrders($user_id){
		...
		if($man->CheckAccess($user_id,'w',589)){
			$ts=new mysqlSet('select count(distinct id) from memo where id in(select distinct memo_id from memo_history where user_id<>"'.$user_id.'" and is_new=1)');
			 ...
			 $ts=new mysqlSet('select count(distinct id) from memo where (user_id="'.$user_id.'" OR user_id="0") and id in(select distinct memo_id from memo_history where user_id<>"'.$user_id.'" and is_new=1) /*and status_id<>4*/');
	...
	*/
	...
	public function ToggleRead($order_id, $user_id,$force_update=true){
		...
		if($force_update){
			...
			$sql='select count(*) from '.$this->tablename.' where '.$this->subkeyname.'="'.$order_id.'" and user_id<>"'.$oi['user_id'].'"  ';
			$s=new mysqlset($sql);
