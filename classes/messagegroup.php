...
class MessageGroup extends AbstractGroup {
	...
	public function DrawAdrArr($current_id, $is_admin=false, $sort_mode=1){
		$arr=array();
		$flt='';
		$flt=' and p.is_active=1  and p.group_id<>2 ';
		
		$srt='';
		if($sort_mode==1) $srt=' order by p.login asc, p.name_s asc';
		elseif($sort_mode==2) $srt=' order by p.name_s asc, p.login asc';
	...
	//	if(!$is_admin) $flt=' and (group_id=1 or group_id=2)';
		$set=new mysqlSet('select p.*, d.name as position_s from user as p left join user_position as d on p.position_id=d.id where p.id<>"'.$current_id.'" '.$flt.' '.$srt);
