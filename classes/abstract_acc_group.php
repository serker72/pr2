...
class AbstractAccGroup extends AbstractGroup {
	...
	public function ShowPos($bill_id, $template, DBDecorator $dec,$can_edit=false, $can_delete=false, $can_confirm=false, $can_super_confirm=false, $has_header=true, $is_ajax=false, $can_restore=false,$limited_sector=NULL,$by_is=NULL, $can_unconfirm=false, &$alls){
		...
		for($i=0; $i<$rc; $i++){
			...
			$bpg=$this->_posgroup->GetItemsByIdArr($f['id'], 0, true, true, false);//$_bpg->GetItemsByIdArr($f['id'], 0, true, true, false);
		//print_r($bpg);
...
/*******************************************************************************************************************/	
	...
	public function ShowAllPos($template, DBDecorator $dec,$can_edit=false, $can_delete=false, $from=0, $to_page=ITEMS_PER_PAGE, $can_confirm=false,  $can_super_confirm=false, $has_header=true, $is_ajax=false, $can_restore=false,$limited_sector=NULL, $can_unconfirm=false){
	...
	//	$_aures=$_au->Auth();
		...
		for($i=0; $i<$rc; $i++){
			...
			$bpg=$this->_posgroup->GetItemsByIdArr($f['id'],0,true,true,false);
		//print_r($bpg);
