...
require_once('doc_out_history_filegroup.php');
require_once('doc_out_history_view_item.php');

// лента Исх док-та
class  DocOut_HistoryGroup extends AbstractGroup {
	...
	public function ShowHistory($order_id, $template, DBDecorator $dec, $can_edit=false, $has_header=true, $is_ajax=false, $result=NULL, $can_edit_record=false, $can_edit_all_record=false, &$data, $do_template=true, $do_read=false, $item=NULL){
		...
		$ofg=new DocOut_HistoryFileGroup;
		...
		$_vi=new DocOut_HistoryViewItem;
	...
	public function CalcNew($user_id, $available_ids, &$first_id){
		...
		$sql='select count(*) from doc_out_history where user_id<>"'.$user_id.'" and user_id<>0
		and sched_id in(
			'.implode(', ',$available_ids).'
		)
		/*and sched_id not in(select id from lead where is_fulfiled=1 or status_id in (36))*/
		 
		and id not in(select history_id from doc_out_history_view where user_id="'.$user_id.'")
		and pdate>"'.$check.'"
		';
		//echo $sql.'<br>';
		...
		if((int)$f[0]>0){
			...
			$sql='select distinct sched_id from doc_out_history where user_id<>"'.$user_id.'" and user_id<>0
			and sched_id in(
				'.implode(', ',$available_ids).'
			)
		/*	and sched_id not in(select id from lead where is_fulfiled=1 or status_id in (36))*/
