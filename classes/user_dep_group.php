...
// группа отделов
class UserDepGroup extends AbstractGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0,  $is_shown=0){
		...
		$_up=new UposDirection;
		//$set=new MysqlSet('select * from '.$this->tablename);
		if($is_shown==0) $set=new MysqlSet('select * from '.$this->tablename.' where parent_id="'.$id.'" order by name asc');
		else $set=new MysqlSet('select * from '.$this->tablename.' where '.$this->vis_name.'="1" and  parent_id="'.$id.'" order by name asc');
		$rs=$set->GetResult();
		$rc=$set->GetResultNumRows();
		for($i=0; $i<$rc; $i++){
			$f=mysqli_fetch_array($rs);
