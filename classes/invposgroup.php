...
class InvPosGroup extends AbstractGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0,$result=NULL){
		...
		for($i=0; $i<$rc; $i++){
			...
			$f['quantity_by_program']=$_pm->FindOstByDate($f['id'], $f['pl_position_id'],date('d.m.Y',$bill['inventory_pdate']),$result['org_id']);
			
			$f['in_acc']=$_ir->CountInAcc($f['id'], $f['pl_position_id'], $result['org_id'],$id);
			$f['in_wf']=$_ir->CountInWf($f['id'], $f['pl_position_id'], $result['org_id'],$id);
