...
class AnBirth{
	...
	public function ShowData($org_id, $supplier_ids,  $user_ids, $limited_user,  $limited_supplier, $pdate1, $pdate2,  $template, DBDecorator $dec2,$pagename='files.php',  $do_it=false, $can_print=false, $can_view_all_suppliers=false, $can_edit_user=false, $can_edit_supplier=false,   &$alls1, &$alls2, $result=NULL, $can_email=false){
		...
		if($do_it){
			...
			foreach($fields as $k=>$v){
				...
				$sql='select c.name, c.position, c.birthdate, c.supplier_id, c.id as contact_id,
		 ...
		 from  supplier_contact as c
			...
			left join supplier_contact_data as scv on scv.contact_id=c.id and scv.kind_id=5
		  ...
		  where s.is_active=1 and c.birthdate is not NULL and s.is_org=0 and s.org_id="'.$org_id.'" '; 
			...
			where   ss.is_active=1 and ss.holding_id in(  '.implode(', ',$supplier_ids).')  )';
				...
				$sql2='select p.*, up.name as position_s,
				count(scv.id) as s_q
				
				
				from user as p
				left join user_position as up on up.id=p.position_id
				left join user_contact_data as scv on scv.user_id=p.id and scv.kind_id=5
				
				 where p.is_active=1 and p.pasp_bithday<>0 ';
				
				
				if($user_ids!==NULL) $sql2.=' and p.id in('.implode(', ', $user_ids).') ';
				
				if($limited_user!==NULL) $sql2.=' and p.id in('.implode(', ', $limited_user).') ';
				
				//фильтр по дате
				if($check_year){
					$sql2.=' and p.pasp_bithday between "'.datefromdmy($pdate1).'"  and "'.datefromdmy($pdate2).'" ';	
				}else{
				...
				//сортировка по дате
				if($check_year){
					$sql2.=' order by  p.pasp_bithday asc';	
				}else{
