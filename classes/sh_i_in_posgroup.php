...
// абстрактная группа
class ShIInPosGroup extends AbstractGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0, $dec2=NULL, $for_an_onway=false, $sh=NULL){
		...
		from '.$this->tablename.' as p 
			...
			left join bill as b on p.out_bill_id=b.id
		where p.'.$this->subkeyname.'="'.$id.'" '.$db_flt.' order by position_name asc, p.out_bill_id asc, id asc';
		
		
		
		/*$sql='select p.id as p_id, p.bill_id,  p.position_id as id, p.position_id as position_id,
					 p.pl_position_id as pl_position_id,
					 p.pl_discount_id, p.pl_discount_value, p.pl_discount_rub_or_percent,
					 d.name, 
		...
		$sql='select p.id as p_id, p.sh_i_id, p.komplekt_ved_pos_id, p.position_id as id, p.position_id as position_id,
					 p.name as position_name, p.dimension as dim_name, 
		...
		*/
		...
		for($i=0; $i<$rc; $i++){
			...
			$f['in_acc']=round($_mf->MaxInAcc($sh['bill_id'], $f['position_id'], $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'], $f['pl_discount_rub_or_percent'],0, $id,$f['out_bill_id']), 3);
			...
			$f['in_bill']=round($_mf->MaxInBill($sh['bill_id'], $f['position_id'],  $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'],  $f['pl_discount_rub_or_percent'],$f['out_bill_id']),3);
			
			$f['not_in_bill']=round($f['in_bill']- $_mf->MaxInShI($sh['bill_id'], $f['position_id'], $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'], $f['pl_discount_rub_or_percent'], NULL,$f['out_bill_id']),3);
