...
// абстрактная группа
class MemoUserGroup extends AbstractGroup {
	...
	public function GetConfirmsbyId($id, $user_id, $result=NULL, $can_confirm=false, $can_super_confirm=false, $can_unconfirm=false, $can_super_unconfirm=false, $can_not_to_next=false){
		...
		$sql='select 
		 ...
		 t.memo_id, 	t.user_id,  	t.ord, 	t.is_confirmed, 	t.confirm_pdate, 	t.confirm_id, 	t.is_not_to_next, 	t.not_to_next_pdate, 	t.not_to_next_id,
		
		u.name_s as name_s, u.login as login, u.position_s as position_s, u.id as id, up.name as position_name,
		
		
		c.name_s as c_name_s, c.login as c_login, c.position_s as c_position_s, cup.name as c_position_name,
		n.name_s as n_name_s, n.login as n_login, n.position_s as n_position_s, nup.name as n_position_name
		...
		left join user_position as cup on c.position_id=cup.id
		
		left join user as n on n.id=t.not_to_next_id
		left join user_position as nup on n.position_id=nup.id
		...
		for($i=0; $i<$rc; $i++){
			...
			if($f['is_confirmed']==1){
				$f['confirm_confirmer']=$f['c_position_name'].' '.$f['c_name_s'].' '.$f['c_login'].' '.$f['confirm_pdate'];
				$was_confirmed=true;
			}else $was_confirmed=false;
			
			if($f['is_not_to_next']==1){
				$f['not_to_next_confirmer']=$f['n_position_name'].' '.$f['n_name_s'].' '.$f['n_login'].' '.$f['confirm_pdate'];
			}
