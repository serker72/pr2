...
require_once('tender_history_filegroup.php');
require_once('tender_history_view_item.php');

// лента тендера
class  Tender_HistoryGroup extends AbstractGroup {
	...
	public function ShowHistory($order_id, $template, DBDecorator $dec, $can_edit=false, $has_header=true, $is_ajax=false, $result=NULL, $can_edit_record=false, $can_edit_all_record=false, &$data, $do_template=true, $do_read=false){
		...
		$ofg=new Tender_HistoryFileGroup;
		...
		$_vi=new Tender_HistoryViewItem;
	...
	//сколько новых комментов для данного пол-ля (кроме своих) с учетом правил доступа по данному тендеру
	//по сути не нужна, перенесена в менеджер индикации
	public function CalcNewByDocument($id, $user_id, $available_ids){
		/*$check=time()-7*24*60*60;
		
		$sql='select count(*) from tender_history where user_id<>"'.$user_id.'"
		and sched_id in(
			'.implode(', ',$available_ids).'
		)
		and sched_id="'.$id.'"
		and id not in(select history_id from tender_history_view where user_id="'.$user_id.'" and sched_id="'.$id.'")
		and pdate>"'.$check.'"
		';
		//echo $sql.'<br>';
	...
	//сколько новых комментов для данного пол-ля (кроме своих) с учетом правил доступа по всем тендерам
	public function CalcNew($user_id, $available_ids, &$first_id){
		...
		and sched_id not in(select id from tender where is_fulfiled=1 or status_id in (36,3))
		...
		if((int)$f[0]>0){
			...
			$sql='select distinct sched_id from tender_history where user_id<>"'.$user_id.'" and user_id<>0
			and sched_id in(
				'.implode(', ',$available_ids).'
			)
			and sched_id not in(select id from tender where is_fulfiled=1 or status_id in (36,3)) 
