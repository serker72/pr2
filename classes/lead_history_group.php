...
require_once('lead_history_filegroup.php');
require_once('lead_history_view_item.php');

require_once('pl_currgroup.php');


// лента лида
class  Lead_HistoryGroup extends AbstractGroup {
	
	
	//установка всех имен
	protected function init(){
		$this->tablename='lead_history';
	...
	public function ShowHistory($order_id, $template, DBDecorator $dec, $can_edit=false, $has_header=true, $is_ajax=false, $result=NULL, $can_edit_record=false, $can_edit_all_record=false, &$data, $do_template=true, $do_read=false, $item=NULL){
		...
		$sm->assign('currs', $_curr->GetItemsArr($item['currency_id']));
		
		
		$ofg=new Lead_HistoryFileGroup;
	...
	public function CalcNew($user_id, $available_ids, &$first_id){
		...
		and sched_id not in(select id from lead where is_fulfiled=1 or status_id in (36,3))
		...
		if((int)$f[0]>0){
			...
			$sql='select distinct sched_id from lead_history where user_id<>"'.$user_id.'" and user_id<>0
			and sched_id in(
				'.implode(', ',$available_ids).'
			)
			and sched_id not in(select id from lead where is_fulfiled=1 or status_id in (36,3))
