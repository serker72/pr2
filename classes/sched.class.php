...
class Sched_MissionItem extends Sched_AbstractItem{
	...
	public function ConstructContacts($id, $item=NULL){
		...
		if($item===NULL) $item=$this->GetItemById($id);
			...
			/*$sql='
			select
				opf.name as opf_name, s.full_name, sc.name, sc.position, sci.value
			from
				sched_contacts as sci
				inner join supplier as s on s.id=sci.supplier_id
				inner join opf on opf.id=s.opf_id
				inner join supplier_contact as sc on sc.id=sci.contact_id
				
			where sci.sched_id="'.$id.'"'	;
			...
			$res=SecStr($f['opf_name'].' '.$f['full_name'].', '.$f['name'].', '.$f['position']).': '.$f['value'];*/
...
class Sched_CallItem extends Sched_AbstractItem{
	...
	public function ConstructContacts($id, $item=NULL, $do_href=false){
		...
		if($item['contact_mode']==0){
			...
			$sql='
			select
				opf.name as opf_name, s.full_name, sc.name, sc.position, sci.value, s.id as supplier_id
			from
				sched_contacts as sci
				inner join supplier as s on s.id=sci.supplier_id
				inner join opf on opf.id=s.opf_id
			...
			if(!$do_href) $res=SecStr($f['opf_name'].' '.$f['full_name'].', '.$f['name'].', '.$f['position']).': '.$f['value'];
			else  $res='<a href="supplier.php?action=1&id='.$f['supplier_id'].'" target="_blank">'.SecStr($f['opf_name'].' '.$f['full_name'].', '.$f['name'].', '.$f['position']).': '.$f['value'].'</a>';
			*/
...
class  Sched_Group extends AbstractGroup {
	...
	public function GetAvailableUserIds($user_id, $except_me=false, $kind_id=1){
		...
		if($testv!==false){
			...
			if(!in_array($user_id, array(1,2))) $sql.=' where id<>1 ';
			$set=new mysqlSet($sql);
			$rs=$set->GetResult();
			$rc=$set->GetResultNumRows();
			...
			//координаторы имеют доступы к встречам, ком-кам всех сотрудников
			$_ui=new UserSItem;
			$ui=$_ui->GetItemById($user_id);
			if(eregi('Координатор', $ui['position_name'])&&(($kind_id==2)||($kind_id==3))){
			...
			//руководители отдела имеют доступ к документам подчиненных сотрудников
			$ui=$_ui->GetItemById($user_id);
			if(eregi('Руководитель отдела', $ui['position_name'])){
			
			
				$sql='select id from user where department_id="'.$ui['department_id'].'"';
				if(!in_array($user_id, array(1,2))) $sql.=' and id<>1 ';
				$set=new mysqlSet($sql);
				$rs=$set->GetResult();
				$rc=$set->GetResultNumRows();
			...
			if(!in_array($user_id, array(1,2))) $sql.=' and allowed_id<>1 ';
			$set=new mysqlSet($sql);
			$rs=$set->GetResult();
			$rc=$set->GetResultNumRows();
...
class SchedRemindGroup extends AbstractGroup {
	...
	public function CalcNewPlans($user_id){
		...
		$sql='select count(distinct s.id) from sched_reminds as r
		inner join sched as s on s.id=r.sched_id
...
class Sched_ViewGroup extends AbstractGroup {
	...
	public function ForCard($user_id, $template, $can_edit=false, $is_ajax=false){
		...
		$sql='select distinct u.id, u.name_s, u.login, u.is_active, up.name as position_s
			from '.$this->tablename.' as p
	...
	public function GetItemsByIdArr($id){
		...
		$sql='select p.*, u.name_s, u.login, up.name as position_s
		from '.$this->tablename.' as p
...
class Sched_WorkingGroup extends Abstract_WorkingGroup {
