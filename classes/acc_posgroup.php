...
class AccPosGroup extends AbstractGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0,$has_goods=true,$has_usl=true, $show_statiscits=true, $show_boundaries=true){
		...
		for($i=0; $i<$rc; $i++){
			...
			if($show_statiscits){
			 //	echo ' zzzzZZZ ';
			  $f['in_bill']=round($_mf->MaxInBill($ac['bill_id'], $f['id'],  $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'], $f['pl_discount_rub_or_percent'],NULL, $f['kp_id']),3);
				  $f['in_rasp']=round($_mf->MaxInShI($ac['bill_id'], $f['id'], $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'], $f['pl_discount_rub_or_percent'], $ac['sh_i_id'], NULL,  $f['kp_id']),3);
				  $f['in_acc']=round($_mf->MaxInAcc($ac['bill_id'],  $f['position_id'], $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'], $f['pl_discount_rub_or_percent'], $id, $ac['sh_i_id'], NULL,  $f['kp_id']),3);
			}
			...
			if(($ac['inventory_id']==0)&&($ac['interstore_id']==0)){
				if($show_statiscits){
				  $f['max_quantity']=$_mf->MaxForAcc($ac['sh_i_id'], $f['id'], $id,   $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'],  $f['pl_discount_rub_or_percent'],NULL,$f['kp_id']);
			  
				  //всего в соответствующей строке счета
				  $f['max_bill_quantity']=$_mf->MaxInBill($ac['bill_id'],$f['id'],   $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'], $f['pl_discount_rub_or_percent'],NULL, $f['kp_id']);
