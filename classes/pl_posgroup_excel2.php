...
//  группа прайс-лист
class PlPosGroupExcel2 extends AbstractGroup {
	...
	public function ShowPos(&$alls, $template, DBDecorator $dec, DBDecorator $decorator_for_equipment,$from=0,$to_page=ITEMS_PER_PAGE,
		$can_edit=false, 
		...
		if($doShow==1){
			...
			for($i=0; $i<$rc; $i++){
				...
				//найти цены ddpm, exw
				/*
				$f['price_sale_exw']=$_pi->CalcPriceF($f['pl_id'],$f['currency_id'],NULL,NULL,NULL,0,0,2,NULL,0);
				
				$f['price_sale_ddpm']=$_pi->CalcPriceF($f['pl_id'],$f['currency_id'],NULL,NULL,NULL,0,0,1,NULL,0);
				
				$f['profit_sale_exw']=round(100*($f['price_sale_exw']-$f['price_f'])/$f['price_sale_exw'],2);
				$f['profit_sale_ddpm']=round(100*($f['price_sale_ddpm']-$f['price_ddpm'])/$f['price_sale_ddpm'],2);
				...
				*/
				...
				//найдем опции:
				 ...
				 $f['options']=$this->ShowOptionsArr($f['id'], $f['currency_id'], $current_price_kind_id);
				...
				$f['calculation_form_id']=$calculation_form_id;
...
//		var_dump($alls); 
	...
	public function GainSql(&$sql, &$sql_count, $currency_id=CURRENCY_DEFAULT_ID, $price_kind_id=PRICE_KIND_DEFAULT_ID){
		$sql='select pl.id as pl_id, pl.discount_id, pl.discount_value, pl.discount_rub_or_percent, pl.position_id, 
					...
					p.is_install, p.is_mandatory,  pl.delivery_ddpm, pl.duty_ddpm, pl.svh_broker, p.pre_quantity, pl.delivery_value, pl.delivery_rub, pl.customs, pl.broker_costs, p.producer_id,
	...
	public function ShowOptionsArr($id,  $currency_id=CURRENCY_DEFAULT_ID, $price_kind_id=PRICE_KIND_DEFAULT_ID, $base_pki=NULL, $base_pi=NULL, $for_pdf=false, $decorator=NULL){
		...
		for($i=0; $i<$rc; $i++){
			...
			//if(!$for_pdf) $f['discs1']=$_dgv->GetItemsByKindPosArr($f['pl_id'], $price_kind_id);  
