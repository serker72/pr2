...
class InvItem extends AbstractItem{
	...
	public function DocCanAnnul($id,&$reason,$item=NULL){
		...
		if($item['status_id']!=1){
		  ...
		  foreach($arr as $k=>$v){
			  ...
			  if($v['is_confirmed']==1) {
				  $can=$can&&false;
				  $reasons[]=' распоряжение на отгрузку № '.$v['id'].' статус документа: '.$dsi['name'];	
			  }
			  
		  }
		  if(count($reasons)>0) $reason.='<br />По распоряжению на инвентаризацию имеются утвержденные распоряжения на отгрузку: ';
		  $reason.=implode('<br /> ',$reasons);
		  ...
		  foreach($arr as $k=>$v){
			  ...
			  if($v['is_confirmed']==1) {
				  $can=$can&&false;
				  $reasons[]=' распоряжение на приемку № '.$v['id'].' статус документа: '.$dsi['name'];	
			  }
			  
		  }
		  if(count($reasons)>0) $reason.='<br />По распоряжению на инвентаризацию имеются утвержденные распоряжения на приемку: ';
		  $reason.=implode('<br /> ',$reasons);
		  ...
		  foreach($arr as $k=>$v){
			  ...
			  if($v['status_id']==2) {
				  $can=$can&&false;
				  $reasons[]=' распоряжение на списание № '.$v['id'].' статус документа: '.$dsi['name'];	
			  }
			  
		  }
		  if(count($reasons)>0) $reason.='<br />По распоряжению на инвентаризацию имеются утвержденные распоряжения на списание: ';
		  $reason.=implode('<br /> ',$reasons);*/
	...
	public function GetBindedDocumentsToAnnul($id){
		...
		//проверить связанные поступления
		  ...
		  if(count($reasons)>0) $reason.='\nпо счету имеются неутвержденные распоряжения на отгрузку: ';
		  $reason.=implode(', ',$reasons);
		  ...
		  if(count($reasons)>0) $reason.='\nпо счету имеются неутвержденные распоряжения на приемку: ';
		  $reason.=implode(', ',$reasons);
	...
	public function AnnulBindedDocuments($id, $_result=NULL){
		...
		$set=new MysqlSet('select * from sh_i where is_incoming=1 and  inventory_id="'.$id.'"');
		$rs=$set->GetResult();
		$rc=$set->GetResultNumRows();
		for($i=0; $i<$rc; $i++){
			$f=mysqli_fetch_array($rs);
			
			$log->PutEntry($_result['id'],'аннулирование распоряжения на приемку в связи с аннулированием инвентаризационного акта',NULL,626,NULL,'распоряжение № '.$f['id'].': установлен статус '.$stat['name'],$f['bill_id']);
			
			
			$log->PutEntry($_result['id'],'аннулирование распоряжения на приемку в связи с аннулированием инвентаризационного акта',NULL,654,NULL,'распоряжение № '.$f['id'].': установлен статус '.$stat['name'],$f['id']);
		}	
		
		$ns=new NonSet('update sh_i set status_id=3 where is_incoming=1 and inventory_id="'.$id.'"');
		
		
		
		$set=new MysqlSet('select * from sh_i where is_incoming=0 and  inventory_id="'.$id.'"');
		$rs=$set->GetResult();
		$rc=$set->GetResultNumRows();
		for($i=0; $i<$rc; $i++){
			$f=mysqli_fetch_array($rs);
			
			$log->PutEntry($_result['id'],'аннулирование распоряжения на отгрузку в связи с аннулированием инвентаризационного акта',NULL,94,NULL,'распоряжение № '.$f['id'].': установлен статус '.$stat['name'],$f['bill_id']);
			
			
			$log->PutEntry($_result['id'],'аннулирование распоряжения на отгрузку в связи с аннулированием инвентаризационного акта',NULL,226,NULL,'распоряжение № '.$f['id'].': установлен статус '.$stat['name'],$f['id']);
		}	
		
		$ns=new NonSet('update sh_i set status_id=3 where is_incoming=0 and  inventory_id="'.$id.'"');
	...
	public function DocCanUnconfirmShip($id,&$reason,$item=NULL, $periods=NULL){
		...
		if($item['is_confirmed_inv']!=1){
		  ...
		  if(count($reasons)>0) {
			 if(strlen($reason)!=0) $reason.=', ';
			  $reason.=' по акту имеются утвержденные распоряжения на приемку: ';
			 // if(strlen($reason)>0) $reason.=',';
		  }
		  $reason.=implode(', ',$reasons);
		  ...
		  if(count($reasons)>0) {
			 if(strlen($reason)!=0) $reason.=', ';
			  $reason.=' по акту имеются утвержденные распоряжения на отгрузку: ';
			 // if(strlen($reason)>0) $reason.=',';
		  }
		  $reason.=implode(', ',$reasons);
	...
	public function CheckInventoryPdate($pdate, &$rss, $except_id=0){
		$res=true; //все ок
		$_dsi=new DocStatusItem;
		
		$sql='select * from '.$this->tablename.' where inventory_pdate>="'.$pdate.'" and id<>"'.$except_id.'" and status_id in(2,16)';
