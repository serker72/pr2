...
// users S
class SuppliersGroup2 extends AbstractGroup {
	protected $group_id;
	protected $is_org;
	...
	public function GetItems($template, DBDecorator $dec, $from=0,$to_page=ITEMS_PER_PAGE,$is_ajax=false, $can_print=false, $limited_supplier=NULL, $result=NULL, $can_merge=false, &$alls){
		$txt='';
		...
		/*	$sri_1=$_sri->GetActual($f['id'], 1);
			...
			*/
			...
			$f['contacts']=		$rg->GetItemsByIdArr($f['id']);
	...
	public function GetItemsWithOpfArr($only_sups=false){
		...
		$sql.=' where p.is_active=1 '.$flt.' order by p.full_name asc';
	...
	//Отбор поставщиков для карт выбора
	public function GetItemsForSelect($template, DBDecorator $dec, $is_ajax=false, &$alls,$resu=NULL, $current_id=0){
		$txt='';
		...
		$sql.='(( p.is_org=0 and p.is_active=1 )) ';
