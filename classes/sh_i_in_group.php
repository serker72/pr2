...
// абстрактная группа
class ShIInGroup extends AbstractShIGroup {
	...
	protected function init(){
		$this->tablename='sh_i';
		$this->pagename='ed_ship_in.php';		
		$this->subkeyname='bill_id';	
		$this->vis_name='is_shown';		
		
		$this->_item=new ShIInItem;
	...
	public function AutoAnnul($days=30, $days_after_restore=30, $annul_status_id=3){
		...
		$set=new MysqlSet('select * from '.$this->tablename.' where is_incoming="'.$this->is_incoming.'" and  status_id<>'.$annul_status_id.' order by id desc');
		...
		for($i=0; $i<$rc; $i++){
			...
			if($can_annul){
				...
				$log->PutEntry(0,'автоматическое аннулирование распоряжения на приемку',NULL,654,NULL,'№ документа: '.$f['id'].' установлен статус '.$stat['name'],$f['id']);
	...
	public function AutoEq($days=21){
		...
		$set=new MysqlSet('select * from '.$this->tablename.' where is_incoming="'.$this->is_incoming.'" and  status_id="7" order by id desc');
		...
		for($i=0; $i<$rc; $i++){
			...
			if($can_annul){
				...
				for($j=0; $j<$rc2; $j++){
					...
					//$args=array();
					$args[]=$h['position_id'].';'.$h['pl_position_id'].';'.$h['pl_discount_id'].';'.$h['pl_discount_value'].';'.$h['pl_discount_rub_or_percent'].';'.$h['quantity'].';'.$h['out_bill_id'];	
