...
class AuthUser{
	...
	public function __construct($sess_name='aet_user'){
		$this->init($sess_name);
	}
	...
	public function AuthUser($login,$password, $fix_activity=true, $build_rights=true){
		$us=new mysqlSet("select u.*, pos.name as position_name from user as u left join user_position as pos on pos.id=u.position_id where u.login=\"".SecStr($login,10)."\" and u.password=\"".SecStr($password,10)."\" and u.is_active=1;");
