...
class KpPosGroup extends AbstractGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0, $show_statistics=true, $bill=NULL){
		...
		$_pg=new PosGroupGroup;
		
		
		$sql='select p.id as p_id, p.kp_id,  p.position_id as id, p.position_id as position_id,
					 p.pl_position_id as pl_position_id,
					 p.pl_discount_id, p.pl_discount_value, p.pl_discount_rub_or_percent, p.delivery_ddpm,
					 p.extra_charges,
					 d.name, 
					 
					 p.name as position_name, p.dimension as dim_name, 
					 p.quantity, p.price, p.price_f, p.price_pm, p.total, p.price_kind_id,
					 p.currency_id as currency_id, p.parent_id, pos.code,
					 curr.signature,
					 pos.producer_id,
					  pos.is_mandatory,
					 ...
					 pos.group_id as two_group_id, parent_gr.id as parent_group_id, parent_gr.parent_group_id as parent_group_parent_group_id, 
					 p.print_form_has_komplekt
		...
		from '.$this->tablename.' as p 
			...
			left join pl_currency as curr on curr.id=p.currency_id	
			left join pl_price_kind as pr_kind on pr_kind.id=p.price_kind_id
		...
		for($i=0; $i<$rc; $i++){
			...
			if($f['parent_group_parent_group_id']==0){
				$f['three_group_id']=0;
			}else{
				  $f['three_group_id']=$f['two_group_id'];
				  $f['two_group_id']=$f['parent_group_id'];
				 $f['parent_group_id']=$f['parent_group_parent_group_id']; 
			}
		...
		//	  $f['in_rasp']=round($_mf->MaxInShI($id, $f['position_id'], $f['pl_position_id'], $f['pl_discount_id'], $f['pl_discount_value'], $f['pl_discount_rub_or_percent'],   NULL),3);
			...
			$max_vals=array();
			$max_vals=$_pdm->GetItemsByKindPosArr($f['pl_position_id'], $f['price_kind_id'] ); //GetItemsByIdArr($f['pl_position_id']);
			$f['discs1']=$max_vals;
