...
class SuppliersMerge{
	...
	public function Merge($data, $result=NULL){
		...
		*/
		...
		new NonSet('update  tender_suppliers set supplier_id="'.$data['base_id'].'"  where  supplier_id in('.implode(', ',$data['ids']).')');
		
		new NonSet('update  lead_suppliers set supplier_id="'.$data['base_id'].'"  where  supplier_id in('.implode(', ',$data['ids']).')');
		
		new NonSet('update  tz_suppliers set supplier_id="'.$data['base_id'].'"  where  supplier_id in('.implode(', ',$data['ids']).')');
		
		new NonSet('update  kp_in_suppliers set supplier_id="'.$data['base_id'].'"  where  supplier_id in('.implode(', ',$data['ids']).')');
		
		 
		
		//документооборот: исход, вход,
		new NonSet('update  doc_in_suppliers set supplier_id="'.$data['base_id'].'"  where  supplier_id in('.implode(', ',$data['ids']).')');
		
		new NonSet('update  doc_out_suppliers set supplier_id="'.$data['base_id'].'"  where  supplier_id in('.implode(', ',$data['ids']).')');
