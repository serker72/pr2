...
//неоптимизированный отчет. использовать, если обнаружатс€ баги в оптимизированном
class AnSupplierOld{
		  ...
		  foreach($sg as $k=>$v){
			 ...
			 // echo count($_dog_arr);
			  ...
			  foreach($_dog_arr as $dk=>$dv){			
				  ...
				  where is_confirmed=1 and is_incoming=1 and (given_pdate between "'.$pdate1.'" and "'.$pdate2.'") and  org_id="'.$org_id.'" and bill_id in(select id from bill where supplier_id="'.$v['id'].'" and org_id="'.$org_id.'" and is_confirmed_price=1 '.$contract_flt.')
				  )
				  UNION ALL
				  ...
				  where is_confirmed=1 and is_incoming=0 and (given_pdate between "'.$pdate1.'" and "'.$pdate2.'") and  org_id="'.$org_id.'" and bill_id in(select id from bill where supplier_id="'.$v['id'].'" and org_id="'.$org_id.'" and is_confirmed_price=1 '.$contract_flt.')
				  )
				  UNION ALL
	...
	public function OstBySup($supplier_id, $pdate, $org_id, $except_invcalc_id=NULL){
		...
		//нам нужно перебрать поступлени€, инввзр, и оплаты, вход€щие в указанный период
			...
			(select id, pdate, "0" as code, "2" as kind, "0" as value, given_pdate, given_no, "0" as debt_id
			 from acceptance
			where is_confirmed=1 and is_incoming=1 and given_pdate<"'.$pdate.'" and  org_id="'.$org_id.'" and bill_id in(select id from bill where org_id="'.$org_id.'" and supplier_id="'.$supplier_id.'")
			)
			UNION ALL
			(select id, pdate, code, "1" as kind, value, given_pdate, given_no, "0" as debt_id from payment
			...
			(select id, pdate, "0" as code, "5" as kind, "0" as value, given_pdate, given_no, "0" as debt_id
			 from acceptance
			where is_confirmed=1 and is_incoming=0 and given_pdate<"'.$pdate.'" and  org_id="'.$org_id.'" and bill_id in(select id from bill where org_id="'.$org_id.'" and supplier_id="'.$supplier_id.'")
			)
			UNION ALL
			(select id, pdate, code, "4" as kind, value, given_pdate, given_no, "0" as debt_id from payment
	...
	public function OstBySupContract($supplier_id, $pdate, $org_id, $contract_id, $except_invcalc_id=NULL){
		...
		//нам нужно перебрать поступлени€, инввзр, и оплаты, вход€щие в указанный период
			...
			(select id, pdate, "0" as code, "2" as kind, "0" as value, given_pdate, given_no, "0" as debt_id
			 from acceptance
			where is_confirmed=1 and is_incoming=1 and given_pdate<"'.$pdate.'" and  org_id="'.$org_id.'" and bill_id in(select id from bill where org_id="'.$org_id.'" and supplier_id="'.$supplier_id.'" and contract_id="'.$contract_id.'")
			)
			UNION ALL
			(select id, pdate, code, "1" as kind, value, given_pdate, given_no, "0" as debt_id from payment
			...
			(select id, pdate, "0" as code, "5" as kind, "0" as value, given_pdate, given_no, "0" as debt_id
			 from acceptance
			where is_confirmed=1 and is_incoming=0 and given_pdate<"'.$pdate.'" and  org_id="'.$org_id.'" and bill_id in(select id from bill where org_id="'.$org_id.'" and supplier_id="'.$supplier_id.'"  and contract_id="'.$contract_id.'")
			)
			UNION ALL
			(select id, pdate, code, "4" as kind, value, given_pdate, given_no, "0" as debt_id from payment
