...
//библиотека классов "станки у контрагента"


//абстрактная запись станок у контарегента
...
//Запрос о возм снятия утв цен
	public function DocCanUnconfirmPrice($id,&$reason,$item=NULL, $periods=NULL){
		...
		if($item['is_confirmed']!=1){
			...
			$can=$can&&false;
			$reasons[]='у станка контрагента не утверждено заполнение';
			$reason.=implode(', ',$reasons);
		} 
	...
	public function DocCanConfirmPrice($id,&$reason,$item=NULL, $periods=NULL){
		...
		if($item['is_confirmed']!=0){
			...
			$can=$can&&false;
			//$dsi=$_dsi->GetItemById($item['status_id']);
			$reasons[]='у станка контрагента утверждено заполнение';
			$reason.=implode(', ',$reasons);
		}else{
	...
	public function ScanDocStatus($id, $old_params, $new_params, $item=NULL, $_result=NULL){
		...
		if(isset($new_params['is_confirmed'])&&isset($old_params['is_confirmed'])){
			if(($new_params['is_confirmed']==1)&&($old_params['is_confirmed']==0)){
				...
				$log->PutEntry($_result['id'],'смена статуса станка контрагента',NULL,87,NULL,'установлен статус '.$stat['name'],$item['id']);
...
/***********************************************************************************************/
//определение класса записи станка контрагента
...
/***************************************************************************************************/
//абстрактная группа станка контрагента
class SupplierTool_AbstractGroup extends AbstractGroup{
	...
	public function ShowPos(
		...
		$sm->assign('unview', $this->_view->GetColsUnArr($this->_auth_result['id']));
			...
			//получим список тех, кто может снять утверждение 
		$_usg1=new UsersSGroup;
