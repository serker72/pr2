...
class TrustPosGroup extends AbstractGroup {
	...
	public function GetItemsByIdArr($id, $current_id=0){
		...
		$sql='select p.id as p_id, p.trust_id, p.position_id as id, p.position_id as position_id, p.pl_position_id as pl_position_id, p.bill_id,
	...
	public function GetItemsByIdPrintArr($id, $current_id=0){
		...
		$_bpf=new BillPosPMFormer;
		$sql='select p.id as p_id, p.trust_id, p.position_id as id,  p.pl_position_id as pl_position_id, p.position_id as position_id, sum(p.quantity) as summa,
					 p.name as position_name, p.dimension as dim_name, 
					 p.quantity, p.price,
					 pd.id as dimension_id,
					 pm.plus_or_minus, pm.value, pm.rub_or_percent			 
		...
		from '.$this->tablename.' as p 
			left join trust_position_pm as pm on pm.trust_position_id=p.id
			left join catalog_dimension as pd on pd.name=p.dimension
		where p.'.$this->subkeyname.'="'.$id.'" group by p.position_id, p.pl_position_id order by position_name asc, p_id asc';
		$set=new MysqlSet($sql);
	...
	public function GetOtherItemsByIdArr($id, $current_id=0){
		...
		for($i=0; $i<$rc; $i++){
			...
			$itms=$_tpi->GetOtherPosArr($f['id'], $f['pl_position_id'], $f['bill_id'], $id);
