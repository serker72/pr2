...
require_once('petition_view.class.php');

require_once('user_s_group.php');
require_once('user_s_item.php');

require_once('authuser.php');
require_once('petitionnotesitem.php');

// абстрактная группа заявлений
class PetitionGroup extends AbstractGroup {
	public $_thg;
	public $_tblink;
	...
	protected function init(){
		...
		$this->pagename='petition.php';		
		$this->subkeyname='order_id';	
		$this->vis_name='is_shown';		
		
//		$this->_thg=new PetitionHistoryGroup;
	...
	public function ShowPos($user_id, //0
	 ...
	 $can_unconfirm_dir=false  //19
		...
		for($i=0; $i<$rc; $i++){
			...
			$f['time_h']=sprintf("%02d", $f['time_h']);
			$f['time_m']=sprintf("%02d", $f['time_m']);
			...
			/*$is_new=$this->_tblink->OverallBlink($f['id'],$color,$f,$can_confirm_chief, $can_unconfirm_chief, $result);		
			$f['color']=$color;
			$f['is_new']=$is_new;
			*/
	...
	public function GetAvailableDocIds($user_id){
		...
		if($_man->CheckAccess($user_id,'w',727)){
			...
			//координаторы имеют доступы к заявлениям всех сотрудников
			$_ui=new UserSItem;
			$ui=$_ui->GetItemById($user_id);
			
			 
			if(eregi('Координатор', $ui['position_name'])){
			...
			if($upos['is_ruk_otd']==1){
			 	$sql.=' or p.manager_id in(select id from user where department_id="'.$user['department_id'].'") ';
			...
			//отдел персонала - видит все заявления со всеми 3мя утверждениями
			$dec=new DBDecorator();
		//.dep.name
			...
			if(in_array($user_id, $uids)){
				$sql.=' or (p.is_confirmed=1 and p.is_ruk=1 and p.is_dir=1)';
				 
			}
			
			
			//отдел АХО - видит все заявления на местную ком-ку со всеми 3мя утверждениями
			$dec=new DBDecorator();
		//.dep.name
			...
			if(in_array($user_id, $uids)){
				$sql.=' or (p.kind_id=6 and p.is_confirmed=1 and p.is_ruk=1 and p.is_dir=1)';
	...
	protected function ConstructNewList($user_id){
		...
		*/	
		...
		//индикация: есть право утв в роли рук-ля 831 - есть док-ты в статусе    41
		 if(
		 	eregi('Руководитель отдела',   $upos['name'])||
		 	$man->CheckAccess($user_id,'w',831)	
		){
			$add=''; 
			if(!$man->CheckAccess($user_id,'w',831)) $add=' and m.department_id="'.$user['department_id'].'" ';
			...
			//	echo $sql;
				...
				$this->new_list[]=array(
					...
					'num'=>(int)$rc,
						...
						'doc_counters'=>array(),
						'sub_ids'=>$sub_ids,
						'url'=>'petition_my_history.php?action=1&id={id}',
					'comment'=>'Утвердите заявление в роли руководителя отдела!'
		...
		//индикация: есть право утв в роли ген дир 1135 - есть док-ты в статусе    43
		 if(
		 	eregi('Генеральный директор',   $upos['name'])||
		 	$man->CheckAccess($user_id,'w',1135)	
		){
			$add=''; 
			...
			//	echo $sql;
				...
				$this->new_list[]=array(
					...
					'num'=>(int)$rc,
						...
						'sub_ids'=>$sub_ids,
						'url'=>'petition_my_history.php?action=1&id={id}',
					'comment'=>'Утвердите заявление в роли генерального директора!'
	...
	public function CountNewDocsExtended($user_id, $do_iconv=true){
		...
		if( eregi('Руководитель отдела',   $upos['name'])||
			...
			if((int)$f[0]>0){
				...
				$rc=$set->GetResultNumRows();
				$g=mysqli_fetch_array($rs);		
				$url='petition_my_history.php?action=1&id='.$g['id'].'&from_begin=1';
		...
		if( eregi('Генеральный директор',   $upos['name'])||
			...
			$add=''; 
//			if(!$man->CheckAccess($user_id,'w',1135)) $add=' and m.department_id="'.$user['department_id'].'" ';
		...
		for($i=0; $i<$rc; $i++){
			...
			if($f['is_confirmed']==0){
				...
				if($f['restore_pdate']>0){
					if(($f['restore_pdate']+$days_after_restore*24*60*60)>$now){
						...
						$can_annul=true;	
						$reason='прошло более '.$days_after_restore.' дней с даты восстановления заявления,  документ не утвержден';
					}
				}else{
					...
					if(($f['pdate']+$days*24*60*60)>$now){
						...
						$can_annul=true;
						$reason='прошло более '.$days.' дней с даты создания заявления,  документ не утвержден';
					}
				}
			}
			...
			if($can_annul){
			 ...
			 $this->_item->Edit($f['id'], array('is_confirmed'=>0, 'status_id'=>$annul_status_id));
				...
				$log->PutEntry(0,'автоматическое аннулирование заявления',NULL,827,NULL,'№ документа: '.$f['code'].' установлен статус '.$stat['name'],$f['id']);
				...
				'pdate'=>time(),
				'posted_user_id'=>0,
				'note'=>'Автоматическое примечание: заявление было автоматически аннулировано, причина: '.$reason.'.'
				));
