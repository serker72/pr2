		...
		if(($ship['is_confirmed']==1)&&($ship!==false)){
		  ...
		  foreach($positions as $k=>$v){
			  if(!$this->PosInSh($v,$ship_positions,$find_pos)){
				  $can=$can&&false;
				  $reasons[]='в родительском распоряжении на приемку не найдена позиция '.SecStr($v['position_name']);	
				  continue;	
			  }
			  ...
			  $free=$mf->MaxForAcc($item['sh_i_id'], $v['id'], $id,  $v['pl_position_id'], $v['pl_discount_id'], $v['pl_discount_value'], $v['pl_discount_rub_or_percent'], $v['out_bill_id'] ) ;
			  
						  
			  if(round($v['quantity'],3)>round($free*PPUP,3)){
				  //превышение
				  $can=$can&&false;
				  $reasons[]='количество позиции '.SecStr($v['position_name']).' '.$v['quantity'].' '.SecStr($v['dim_name']).' превышает доступное по распоряжению на приемку ('.round($free*PPUP,3).'  '.SecStr($v['dim_name']).')';	
				  continue;		
			  }
			...
			$can=$can&&false;
				  $reasons[]='не утверждено родительское распоряжение на приемку № '.$item['sh_i_id'];	
