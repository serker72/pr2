...
//абстрактный элемент
class ShIItem extends BillItem{
	public $sh_i_blink;
	public $rd;
	...
	public function ScanDocStatus($id, $old_params, $new_params, $item=NULL, $_result=NULL){
		...
		if(isset($new_params['is_confirmed'])&&isset($old_params['is_confirmed'])){
			...
			if(($new_params['is_confirmed']==1)&&($old_params['is_confirmed']==0)&&($old_params['status_id']==1)){
				...
				$log->PutEntry($_result['id'],'смена статуса распоряжения на отгрузку',NULL,93,NULL,'установлен статус '.$stat['name'],$item['bill_id']);
				
				$log->PutEntry($_result['id'],'смена статуса распоряжения на отгрузку',NULL,219,NULL,'установлен статус '.$stat['name'],$id);
		  ...
		  //переход 2-7 - создали хотя бы 1 реализацию
		  if($item['status_id']==2){
	...
	//запрос о совпадении позиций по подчиненным реализацию
	...
	public function DocCanUnconfirm($id,&$reason,$periods=NULL){
		...
		if($item['is_confirmed']!=1){
			...
			$can=$can&&false;
			$reasons[]='распоряжение на отгрузку не утверждено';
			$reason.=implode(', ',$reasons);
		}else{
		
		  //проверить связанные реализацию
		  $_accg=new AccGroup;
		  ...
		  if(count($reasons)>0) {
			  $reason.=' по распоряжению на отгрузку имеются утвержденные реализации: ';
		...
		if($item['status_id']!=1){
		  ...
		  for($i=0; $i<$rc; $i++){
			  ...
			  if($v['status_id']==5) {
				  ...
				  $can=$can&&false;
				  $reasons[]='связанная реализация <a target=_blank href=ed_acc.php?id='.$v['id'].'&action=1&from_begin=1>№ '.$v['id'].'</a> статус документа: '.$v['name'];	
			  }
	...
	//определить заданную дату последнего утв. реализацию по распоряжению...
	public function GetClosePdate($id, &$descr, $item=NULL){
		...
		if($rc>0){
			$f=mysqli_fetch_array($rs);
			$descr='заданная дата с/ф последней утвержденной реализации по распоряжению (реализация № '.$f[1].', заданный номер '.$f[2].', заданная дата '.date('d.m.Y',$f[0]).')';
			return $f[0];
		}else{
			$descr='плановая дата поставки по распоряжению на отгрузку ';
	...
	//проверка, свободны ли позиции реализацию для восстановления/утверждения
	public function CanConfirmByPositions($id,&$reason,$item=NULL){
		...
		if(($bill!==false)&&($bill['is_confirmed_price']==1)&&($bill['is_confirmed_shipping']==1)){
		  ...
		  foreach($positions as $k=>$v){
			  ...
			  $free=$mf->MaxForShI($item['bill_id'],$v['id'],$id,$item['storage_id'],$item['sector_id'], $v['komplekt_ved_id']) ;
			  
						  
			  if(round($v['quantity'],3)>round($free*PPUP,3)){
				  //превышение
				  $can=$can&&false;
				  $reasons[]='количество позиции '.SecStr($v['position_name']).' '.$v['quantity'].' '.SecStr($v['dim_name']).' превышает доступное по исходящему счету ('.round($free*PPUP,3).'  '.SecStr($v['dim_name']).')';	
				  continue;		
			  }
			...
			$can=$can&&false;
			$reasons[]='родительский исходящий счет №'.$bill['code'].' не утвержден';	
		}
	...
	public function GetBindedDocumentsToAnnul($id){
		...
		foreach($arr as $k=>$v){
			...
			if($v['status_id']==4) {
				...
				$can=$can&&false;
				$reasons[]='связанная реализация № '.$v['id'].' статус документа: '.$dsi['name'];	
			}
	...
	public function AnnulBindedDocuments($id){
		...
		for($i=0; $i<$rc; $i++){
			...
			$log->PutEntry($_result['id'],'аннулирование реализации в связи с аннулированием распоряжения на отгрузку',NULL,94,NULL,'реализация № '.$f['id'].': установлен статус '.$stat['name'],$f['bill_id']);
			
			
			$log->PutEntry($_result['id'],'аннулирование реализации в связи с аннулированием распоряжения на отгрузку',NULL,242,NULL,'реализация № '.$f['id'].': установлен статус '.$stat['name'],$f['id']);
		}	
	...
	//метод для выравнивания по реализации
	public function DoEq($id, array $args, &$output, $is_auto=0, $sh=NULL, $_result=NULL,$extra_reason=''){
		...
		if($sh['is_confirmed']==0){
			$output='Выравнивание позиций невозможно: распоряжение на отгрузку не утверждено.';
			return;
		}
			...
			if($sh_p!==false){
				...
				if($v['delta']>=0){ //выравнивать только недовоз! перевоз не выравнивать!
				  ...
				  ($is_auto==1)&&(round(($v['quantity']-$v['delta']),3)>0)){
					  ...
					  $description='Распоряжение на отгрузку №'.$id.': '.$sh_p['name'].' <br /> Кол-во: '.$v['quantity'].' было изменено на:  '.round($params['quantity'],3).'<br /> ';
					  ...
					  if($is_auto==1){
						  ...
						  $log->PutEntry(NULL,'автоматическое редактирование позиции распоряжения на отгрузку в связи с автоматическим выравниванием позиций',NULL,93,NULL,$description.$extra_reason,$sh['bill_id']);	
					  
					  	  $log->PutEntry(NULL,'автоматическое редактирование позиции распоряжения на отгрузку в связи с автоматическим выравниванием позиций',NULL,219,NULL,$description.$extra_reason,$sh['id']);	
						  
						  $posted_user_id=0;
						  $note='Автоматическое примечание: позиция распоряжения на отгрузку '.$sh_p['name'].' была выровнена при автоматическом выравнивании, кол-во '.$v['quantity'].' было изменено на '.round($params['quantity'],3).''.$extra_reason;
					  }else{
						  
						  $log->PutEntry($_result['id'],'редактировал позицию распоряжения на отгрузку в связи с выравниванием позиций',NULL,93,NULL,$description.$extra_reason,$sh['bill_id']);	
					  
					  	  $log->PutEntry($_result['id'],'редактировал позицию распоряжения на отгрузку в связи с выравниванием позиций',NULL,219,NULL,$description.$extra_reason,$sh['id']);	
				  
						  
						  $posted_user_id=$_result['id'];
						  $note='Автоматическое примечание: позиция распоряжения на отгрузку '.$sh_p['name'].' была выровнена, кол-во '.$v['quantity'].' было изменено на '.round($params['quantity'],3).''.$extra_reason;
					  }
					  $_ni->Add(array(
						'user_id'=>$id,
						'is_auto'=>1,
	...
	public function ScanEq($id, array $args, &$output, $sh=NULL){
		...
		foreach($args as $k=>$v){
			...
			//по каждой позиции перебрать все позиции подчиненных реализации
		...
		if($total_summ==0){
			...
			if($total_summ>$summ_in_doc){
				$output='Позиция '.htmlspecialchars($pos['name']).' найдена в утвержденных реализациях: '.implode(', ',$docs).' в количестве '.$total_summ.' '.htmlspecialchars($pdi['name']).', что превышает количество в распоряжении на отгрузку '.$summ_in_doc.' '.htmlspecialchars($pdi['name']).'. Позиция не подлежит выравниванию.';
				
			}elseif($total_summ==$summ_in_doc){
				$output='Позиция '.htmlspecialchars($pos['name']).' найдена в утвержденных реализациях: '.implode(', ',$docs).' в количестве '.$total_summ.' '.htmlspecialchars($pdi['name']).', что равно количеству в распоряжении на отгрузку '.$summ_in_doc.' '.htmlspecialchars($pdi['name']).'. Позиция не подлежит выравниванию.';
				
			
			}else $output='Позиция '.htmlspecialchars($pos['name']).' '.$summ_in_doc.' '.htmlspecialchars($pdi['name']).' найдена в утвержденных реализациях: '.implode(', ',$docs).' в количестве '.$total_summ.' '.htmlspecialchars($pdi['name']).'. Продолжить выравнивание данной позиции?';
		}
	...
	public function CanEditQuantities($id, &$reason,$itm=NULL){
		...
		if(($itm!==false)&&(($itm['is_confirmed']!=0))) {
			$reason.='распоряжение на отгрузку утверждено';
