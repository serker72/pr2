...
class BillItem extends AbstractItem{
	...
	public function AddPositions($current_id, array $positions,$can_change_cascade=false, $check_delta_summ=false, $result=NULL,$bill=NULL){
		...
		foreach($positions as $k=>$v){
			...
			if($kpi===false){
				...
				if($kpi['kp_id']!=$add_array['kp_id']) $to_log=$to_log||true;
	...
	public function CanDelete($id, &$reason){
		...
		if($rc>0){
			...
			$reason.='по счету имеются распоряжения на отгрузку: ';
		 	$nums=array();
			for($i=0;$i<$rc;$i++){
	...
	public function DocCanAnnul($id,&$reason,$item=NULL){
		...
		if($item['status_id']!=1){
		  ...
		  for($i=0; $i<$rc; $i++){
			 ...
			 // if($v['is_confirmed']==1) {
				  $can=$can&&false;
				  $reasons[]=' распоряжение на отгрузку <a target=_blank href=ed_ship.php?id='.$v['id'].'&action=1&from_begin=1>№ '.$v['id'].'</a> статус документа: '.$v['name'];	
			 // }
			  
		  }
		  if(count($reasons)>0) $reason.="<br />По счету имеются утвержденные распоряжения на отгрузку: ";
		  $reason.=implode('<br /> ',$reasons);
		...
		foreach($items as $k=>$v){
			...
			if($sh_p!==false){
				...
				if($v['delta']>=0){ //выравнивать только недовоз! перевоз не выравнивать!
				  ...
				  //создать примечание 
				  if($is_auto==1){
					 $log->PutEntry(0,'автоматическое редактирование позиции счета в связи с выравниванием позиций',NULL,93,NULL,$description.$extra_reason,$id);	 
					 $posted_user_id=0;
					 ...
					 $log->PutEntry($_result['id'],'редактировал позицию счета в связи с выравниванием позиций',NULL,93,NULL,$description.$extra_reason,$id);	
	...
	public function ScanEq($id, array $args, &$output, $sh=NULL, $express_scan=false, $continue_message=".\nПродолжить выравнивание данной позиции?"){
		...
		if($total_summ==0){
			...
			if(!$express_scan){
			  //в распоряжениях
			  if(count($docs1)){
				   $output.="\nПозиция ".htmlspecialchars($pos["name"])." найдена в утвержденных распоряжениях на отгрузку: ".implode(", ",$docs1)." в количестве ".$count_sh." ".htmlspecialchars($pdi["name"]);
				   
					if($count_sh>$summ_in_doc){
					   $output.=', что превышает количество в cчете '.$summ_in_doc.' '.htmlspecialchars($pdi['name']).'';
				   }
			  }else $output.="\nПозиция ".htmlspecialchars($pos["name"])." не найдена в утвержденных распоряжениях на отгрузку.";
			}
			...
			//в поступлениях 
			if(count($docs)){
				 $output.="\nПозиция ".htmlspecialchars($pos["name"])." найдена в утвержденных реализациях: ".implode(", ",$docs)." в количестве ".$count_acc." ".htmlspecialchars($pdi["name"]);
			...
			}else $output.="\nПозиция ".htmlspecialchars($pos["name"])." не найдена в утвержденных реализациях.";
