	...
	public function ShowData($pdate1, $pdate2, $bank_id, $extended_an=0, $org_id, $template, DBDecorator $dec,$pagename='files.php',  $do_it=false, $can_print=false, $dec_sep=DEC_SEP,&$alls){
		...
		if($do_it){
		  ...
		  for($i=0; $i<$rc; $i++){
			  ...
			  $bank_plus=0;  $bank_minus=0; $doc_begin_ost=0;
			  for($j=0; $j<$rc2; $j++){
			  ...
			  $f['begin_ost']=number_format(0,2,'.',$dec_sep);
			  
			  $f['after_ost']=number_format(round($bank_plus-$bank_minus,2),2,'.',$dec_sep);
	   ...
	   $sm->assign('items',$alls);
		...
		$sm->assign('begin_ost',0.00);
		$sm->assign('after_ost',number_format(round($total_plus-$total_minus,2),2,'.',$dec_sep));	
