...
$smarty->assign("SITETITLE",'Договор контрагента');
...
if(($sup_id==13991)&&!in_array($result['id'], array(1,2))){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
$log->PutEntry($result['id'],'открыл реестр файлов договора контрагента',NULL,91, NULL, $user['code'].' '.$user['full_name'].$_folder_desrc,$sup_id);
...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил договор',NULL,173,NULL,'имя файла '.SecStr($file['orig_name']),$sup_id);
	}
...
$_menu_id=27;
	...
	*/
	...
	'',
	 $au->user_rights->CheckAccess('w',799),
	 $result
	);
