...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',87)){
		...
		$log->PutEntry($result['id'],'создал папку',NULL,87,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',87)){
		...
		$_ff->Edit($id, $params,NULL,$result,87);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,87,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
