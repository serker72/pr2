...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,731,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',1130)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['bill_id']), new MemoFileItem(1));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,1130,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',1130)){
		...
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['bill_id']), new MemoFileItem);
		$_ff->SetTablename('memo_file_folder');
		$_ff->SetDocIdName('bill_id');
		...
		$_ff->Edit($id, $params,NULL,$result,1130);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,1130,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new MemoFileItem, new FileDocFolderItem(1,abs((int)$_POST['bill_id']), new MemoFileItem(1)));   //FileFolderList($storage_id);
	$_ff->setTableName('memo_file_folder');
