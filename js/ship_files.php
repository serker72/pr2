...
require_once('../classes/sh_i_fileitem.php');
...
if(!$au->user_rights->CheckAccess('w',222)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,222,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',221)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['ship_id']), new ShIFileItem(4));
		...
		$log->PutEntry($result['id'],'создал папку',NULL,221,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',222)){
		...
		$_ff->Edit($id, $params,NULL,$result,222);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,222,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['ship_id']), new ShIFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['ship_id']), new ShIFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('sh_i_file_folder');
