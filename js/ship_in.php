...
if(isset($_POST['action'])&&($_POST['action']=="load_positions")){
	...
	$ret.=$sm->fetch("ships_in/positions_edit_set.html");
	...
	$ret=$sm->fetch("ships_in/positions_on_page_set.html");
...
elseif(isset($_POST['action'])&&($_POST['action']=="redraw_notes")){
	...
	$sm->assign('items',$rg->GetItemsByIdArr($user_id,0,0,false,$au->user_rights->CheckAccess('w',646), $au->user_rights->CheckAccess('w',647),$result['id']));
	$sm->assign('word','notes');
	$sm->assign('user_id',$user_id);
	$sm->assign('named','Примечания');
	
	$sm->assign('can_edit', $au->user_rights->CheckAccess('w',645));
	
	
	$ret=$sm->fetch('ships_in/d_notes.html');
	...
	if(!$au->user_rights->CheckAccess('w',645)){
		header("HTTP/1.1 403 Forbidden");
		  header("Status: 403 Forbidden");	
		  die();	
	}
	...
	$log->PutEntry($result['id'],'добавил примечания распоряжению на приемку', NULL,645, NULL,SecStr(iconv("utf-8","windows-1251",$_POST['note']),9),$user_id);
	...
	$log->PutEntry($result['id'],'редактировал примечания по распоряжению на приемку', NULL,645,NULL, SecStr(iconv("utf-8","windows-1251",$_POST['note']),9),$user_id);
	...
	$log->PutEntry($result['id'],'удалил примечания по распоряжению на приемку', NULL,645,NULL,NULL,$user_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_confirm")){
	...
	if($trust['is_confirmed']==1){
		...
		if($au->user_rights->CheckAccess('w',651)||$au->user_rights->CheckAccess('w',96)){
			if($trust['status_id']==1){
				...
				$log->PutEntry($result['id'],'утвердил распоряжения на приемку',NULL,613, NULL, NULL,$bill_id);
				
				$log->PutEntry($result['id'],'утвердил распоряжения на приемку',NULL,651, NULL, NULL,$id);	
	...
	if($shorter==0){
		 $template='ships_in/all_ships_list.html';
		  $prefix='_1';
	}else{
		   $prefix='_ship';
		 $template='ships_in/ships_list.html';
	}
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_annul")){
	...
	if(($trust['status_id']==1)&&($trust['is_confirmed']==0)){
		//удаление	
		if($au->user_rights->CheckAccess('w',654)){
			$_ti->Edit($id,array('status_id'=>3, 'confirm_pdate'=>time(), 'user_confirm_id'=>$result['id']),false,$result);
			
			$stat=$_stat->GetItemById(3);
			$log->PutEntry($result['id'],'аннулирование распоряжения на приемку',NULL,613,NULL,'распоряжение на приемку № '.$trust['id'].': установлен статус '.$stat['name'],$trust['bill_id']);
			
			$log->PutEntry($result['id'],'аннулирование распоряжения на приемку',NULL,654,NULL,'распоряжение на приемку № '.$trust['id'].': установлен статус '.$stat['name'],$id);
		...
		//разудаление
		if($au->user_rights->CheckAccess('w',655)){
			$_ti->Edit($id,array('status_id'=>1, 'confirm_pdate'=>time(), 'user_confirm_id'=>$result['id']),false,$result);
			
			$stat=$_stat->GetItemById(1);
			$log->PutEntry($result['id'],'восстановление распоряжения на приемку',NULL,655,NULL,'распоряжение на приемку № '.$trust['id'].': установлен статус '.$stat['name'],$trust['bill_id']);		
			
			
			$log->PutEntry($result['id'],'восстановление распоряжения на приемку',NULL,227,NULL,'распоряжение на приемку № '.$trust['id'].': установлен статус '.$stat['name'],$id);	
	...
	$au->user_rights->CheckAccess('w',652));
		...
		$editing_user['can_annul']=$_ti->DocCanAnnul($editing_user['id'],$reason)&&$au->user_rights->CheckAccess('w',654);
		...
		$ret=$sm->fetch('ships_in/toggle_annul_card.html');	
	}
}
elseif(isset($_POST['action'])&&($_POST['action']=="find_acc_pos")){
	...
	//$ret=$_kr->InAcc($id,$bill_id, 'ships_in/in_accs.html',$result['org_id'],true,$sh_i_id,$storage_id,$sector_id,$komplekt_ved_id,$au->user_rights->CheckAccess('w',87));
	$ret=$_kr->InAcc($id,$bill_id, 'ships_in/in_accs.html',$result['org_id'],true, $sh_i_id, $pl_position_id, $pl_discount_id, $pl_discount_value,  $pl_discount_rub_or_percent ,$au->user_rights->CheckAccess('w',87),  $out_bill_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="find_sh_pos")){
	...
	$ret=$_kr->InSh($position_id,$bill_id,'ships_in/in_shs.html',$result['org_id'],true,$sh_i_id,   $pl_position_id, $pl_discount_id, $pl_discount_value,  $pl_discount_rub_or_percent ,$au->user_rights->CheckAccess('w',87), $out_bill_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="find_bills_pos")){
	...
	$ret=$_kr->InBills($position_id,  $bill_id,'ships_in/in_bills.html',$result['org_id'], true, $sh_i_id, $pl_position_id, $pl_discount_id, $pl_discount_value, $pl_discount_rub_or_percent, $au->user_rights->CheckAccess('w',87),$out_bill_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_eq")){
	...
	if($au->user_rights->CheckAccess('w',656)){
