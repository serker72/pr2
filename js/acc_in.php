...
if(isset($_POST['action'])&&($_POST['action']=="load_positions")){
	...
	}else $sm->assign('cannot_view_pm',!$au->user_rights->CheckAccess('w',663));
	
	$sm->assign('can_exclude_positions',$au->user_rights->CheckAccess('w',662));
	
	
	$sm->assign('can_add_positions',$au->user_rights->CheckAccess('w',661)||$au->user_rights->CheckAccess('w',664)); 
		$sm->assign('can_del_positions',$au->user_rights->CheckAccess('w',661)||$au->user_rights->CheckAccess('w',664)); 
	
	$sm->assign('pos_change_low_mode',abs((int)$_POST['change_low_mode']));
	$sm->assign('pos_change_high_mode',abs((int)$_POST['change_high_mode']));
	$sm->assign('PPUP',PPUP);
	
	$ret.=$sm->fetch("acc_in/positions_edit_set.html");
	...
	//$sm->assign('cannot_view_pm',!$au->user_rights->CheckAccess('w',675));
	...
	$ret=$sm->fetch("acc_in/positions_on_page_set.html");
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_confirm")){
		...
		if(!$_ti->ParentBillHasPms($trust['id'], $trust)||($trust['inventory_id']!=0)) $can=$can&&$au->user_rights->CheckAccess('w',672);
		  ...
		  if($can){
			  if($trust['status_id']==5){
				  ...
				  $log->PutEntry($result['id'],'снял утверждение поступления товара',NULL,644, NULL, NULL,$trust['sh_i_id']);
		  ...
		  if($au->user_rights->CheckAccess('w',671)||$au->user_rights->CheckAccess('w',96)){
			 if($trust['status_id']==4){
				  ...
				  $log->PutEntry($result['id'],'утвердил поступление товара',NULL,613, NULL, NULL,$bill_id);	
				   ...
				   $log->PutEntry($result['id'],'утвердил поступление товара',NULL,644, NULL, NULL,$trust['sh_i_id']);	
	...
	$ret=$acg->ShowAllPos($template,$dec, $au->user_rights->CheckAccess('w',664)||$au->user_rights->CheckAccess('w',673), $au->user_rights->CheckAccess('w',674),0,100, $au->user_rights->CheckAccess('w',671),  $au->user_rights->CheckAccess('w',96),false,true, $au->user_rights->CheckAccess('w',675),NULL,$au->user_rights->CheckAccess('w',672));
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_annul")){
	...
	if(($trust['status_id']==4)&&($trust['is_confirmed']==0)){
		...
		if($au->user_rights->CheckAccess('w',674)){
			...
			$log->PutEntry($result['id'],'аннулирование поступления',NULL,644,NULL,'поступление № '.$trust['id'].': установлен статус '.$stat['name'],$trust['sh_i_id']);
		...
		if($au->user_rights->CheckAccess('w',675)){
			...
			$log->PutEntry($result['id'],'восстановление поступления',NULL,644,NULL,'поступление № '.$trust['id'].': установлен статус '.$stat['name'],$trust['sh_i_id']);	
...
elseif(isset($_POST['action'])&&($_POST['action']=="find_shs_pos")){
	...
	$ret=$_kr->InAcc($id, $except_id, $bill_id,'acc_in/in_accs.html',$result['org_id'],true,$sh_i_id, $pl_position_id, $pl_discount_id, $pl_discount_value,  $pl_discount_rub_or_percent, $out_bill_id );
