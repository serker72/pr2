...
if(isset($_POST['action'])&&($_POST['action']=="load_positions")){
	...
	$pos=$_ipg->GetItemsByDateStorSec($pdate,$result['org_id'], $current_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_confirm")){
	...
	if($trust['is_confirmed']==1){
		...
		if($au->user_rights->CheckAccess('w',331)){
			if(($_ti->CheckInventoryPdate($trust['inventory_pdate'],$trust['sector_id'],$trust['storage_id'],$rss,$trust['id']))&&($trust['status_id']==1)){
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_annul")){
	...
	$ret=$_kr->InAcc($id, $pl_position_id, 'inv/in_accs_in.html',$result['org_id'],true, $inventory_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="find_wf_pos")){
	...
	$ret=$_kr->InWf($id, $pl_position_id, 'inv/in_accs.html',$result['org_id'],true, $inventory_id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="check_inventory_pdate")){
	...
	$res=$_kr->CheckInventoryPdate($pdate,$rss,$id);
