...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу лида описание',NULL,950,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	 if(!$au->user_rights->CheckAccess('w',950)){
		...
		$_ff=new FileDocFolderItem(2,abs((int)$_POST['id']), new LeadFileItem2(2));
	...
	//	$params['org_id']=$result['org_id'];
		...
		$log->PutEntry($result['id'],'создал папку',NULL,950,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$params['doc_id']);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	 if(!$au->user_rights->CheckAccess('w',950)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,950,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new LeadFileItem2(2), new FileDocFolderItem(2,abs((int)$_POST['bill_id']), new LeadFileItem2(2)));   //FileFolderList($storage_id);
	$_ff->setTableName('lead_file_folder');
