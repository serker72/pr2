...
if(isset($_GET['action'])&&($_GET['action']=="try_annul")){
	...
	$s1=new mysqlSet('select * from  inactual_time_markers where kind="'.$kind.'" order by ptime desc limit 1');
	//echo mysql_error();
	...
	if($rc1>0){
		...
		if(((int)$f1['ptime']+(int)$f1['expiration'])<=time()){
			$s2=new NonSet('delete from inactual_time_markers where id="'.$f1['id'].'"');
			$s3=new NonSet('insert into inactual_time_markers (ptime, expiration, kind) values('.time().', '.$_timeout.', '.$kind.')');
