...
require_once('../classes/taskincominghistorygroup.php');
require_once('../classes/taskoutcominghistorygroup.php');

require_once('../classes/taskallhistorygroup.php');
...
require_once('../classes/memoincominghistorygroup.php');
require_once('../classes/memooutcominghistorygroup.php');

require_once('../classes/memoallhistorygroup.php');



require_once('../classes/petitionblink.php');
...
if(isset($_GET['action'])&&($_GET['action']=="do_doc_in_messages")){
	require_once('../classes/doc_in.class.php');
	 
	$_mes=new DocIn_Messages;
	
	$_mes->SendMessageRemind();
...
if(isset($_GET['action'])&&($_GET['action']=="do_doc_vn_messages")){
	require_once('../classes/doc_vn.class.php');
		...
		if(((int)$v['ptime']+(int)$v['expiration'])<=time()){
			...
			if((($ki['status_id']==1)||($ki['status_id']==11))&&(!$_ki->DocCanUnconfirm($v['komplekt_ved_id']))){
				...
				foreach($positions_to_change as $kk=>$vv){
					$test_kpi=$_kpi->GetItemByFields(array('komplekt_ved_id'=>$v['komplekt_ved_id'], 'position_id'=>$vv['position_id'], 'storage_id'=>$vv['storage_id']));
				...
				//проставить утвержение НОС
				$test_kci=$_kci->GetItemByFields(array('role_id'=>6, 'komplekt_ved_id'=>$v['komplekt_ved_id']));
				if($test_kci===false){
					$_kci->Add(array('role_id'=>6, 'komplekt_ved_id'=>$v['komplekt_ved_id'], 'pdate'=>time(),'user_id'=>$v['user_id']));	
					
					$role=$_kcri->GetItemById(6);
					//в журнал, в примечания...
					$log->PutEntry(0,'автоматическое утверждение заявки',NULL,84,NULL,'роль: '.$role['name'],$v['komplekt_ved_id']);	
					$_kni->Add(array('user_id'=>$v['komplekt_ved_id'], 	'pdate'=>time(), 	'note'=>'Автоматическое примечание: заявка была утверждена в роли '.$role['name'].' автоматически при восстановлении утверждения', 	'posted_user_id'=>0, 	'is_auto'=>1));
				}
...
//проверка, сколько новых тендеров в меню
elseif(isset($_POST['action'])&&($_POST['action']=="total_count_tenders")){
	
	$mg=new Tender_Group;
	 
	
	$count_new=$mg->CountNewTenders($result['id']);
	if($count_new>0) $count_new='('.$count_new.')';
	$ret=$count_new;
...
//проверка, сколько новых лидов в меню
elseif(isset($_POST['action'])&&($_POST['action']=="total_count_leads")){
	
	$mg=new Lead_Group;
	 
	
	$count_new=$mg->CountNewLeads($result['id']);
	if($count_new>0) $count_new='('.$count_new.')';
	$ret=$count_new;
...
//проверка, сколько новых лидов в меню - расширенная версия
elseif(isset($_GET['action'])&&($_GET['action']=="total_count_leads_extended")){
	...
	*/
	...
	$data=$mg->CountNewLeadsExtended($result['id']);
//	if(count($data)>0){
...
//проверка, сколько новых тендеров в меню - расширенная версия
elseif(isset($_GET['action'])&&($_GET['action']=="total_count_tenders_extended")){
	...
	*/
	...
	$data=$mg->CountNewTendersExtended($result['id']);
//	if(count($data)>0){
	$ret=json_encode($data);
	//}
...
//проверка, сколько новых ТЗ в меню
elseif(isset($_GET['action'])&&($_GET['action']=="total_count_tzs")){
	
	$mg=new TZ_Group;
	 
	
	$count_new=$mg->CountNewTzs($result['id']);
	if($count_new>0) $count_new='('.$count_new.')';
	$ret=$count_new;
...
//проверка, сколько новых док вн в меню
elseif(isset($_GET['action'])&&($_GET['action']=="total_count_dvs_extended")){
	
	$mg=new DocVn_Group;
	 
	
	$data=$mg->CountNewDocsExtended($result['id']);
//	if(count($data)>0){
	$ret=json_encode($data);
...
//проверка, сколько новых заявл в меню
elseif(isset($_GET['action'])&&($_GET['action']=="total_count_petitions_extended")){
	
	$mg=new PetitionAllGroup;
...
//проверка, сколько новых сл зап в меню
elseif(isset($_GET['action'])&&($_GET['action']=="total_count_memos_extended")){
