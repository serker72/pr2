	...
	if(($trust['status_id']==18)&&($trust['is_confirmed']==0)){
		...
		if($au->user_rights->CheckAccess('w',950)){
			...
			$log->PutEntry($result['id'],'восстановление лида',NULL,950,NULL,'лид № '.$trust['code'].': установлен статус '.$stat['name'],$id);
			 ...
			 //автоматически поднять  в 1 вероятность
			 $_ti->Edit($id,array('probability'=>1));
			...
			$_ni->Add(array(
				'sched_id'=>$id,
				'user_id'=>0,
				'txt'=>'Автоматический комментарий: документ был восстановлен пользователем '.SecStr($result['name_s']).'',
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_users")){
	...
	$alls=$_si->GetUsersWhoAvailable($supplier_id, NULL, $resp_ids);
