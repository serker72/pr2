...
лиды в статусе в работе, утвержден, на рассмотрении
*/
...
$check_pdate=date('Y-m-d', time()+14*24*60*60);


$sql='select * from user where is_active=1 and id in( select distinct manager_id from lead where status_id in(2,28,35) and pdate_finish<="'.$check_pdate.'")';
...
$topic='Осталось менее 2 недель до прогнозируемой даты заключения контракта по Вашим лидам';
...
foreach($users_to_send as $k1=>$user){
	...
	$txt.='<div>';
	$txt.='<strong>Внимание! По следующим Вашим лидам осталось менее двух недель до прогнозируемой даты заключения контракта:</strong>';
	$txt.='</div>';
	
	$txt.='<div>&nbsp;</div><ul>';
	
	$sql='select * from lead   where status_id in(2,28,35) and pdate_finish<="'.$check_pdate.'"  and manager_id="'.$user['id'].'" ';
	...
	$txt.='<div><strong>Просим изменить статус на корректный: Проигран или Выигран, либо перенести конкурс (перевести лид в статус Отложен)!</strong></div>';
