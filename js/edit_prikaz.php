...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу приказа описание',NULL,809,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',809)){
		...
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['id']), new PrikazFileItem(4));
	...
	//	$params['org_id']=$result['org_id'];
		...
		$log->PutEntry($result['id'],'создал папку',NULL,809,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',93)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,93,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['id']), new PrikazFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['id']), new PrikazFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('prikaz_file_folder');
