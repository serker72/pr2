...
require_once('../classes/sh_i_in_fileitem.php');
...
if(!$au->user_rights->CheckAccess('w',649)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,649,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',648)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(3,abs((int)$_POST['ship_id']), new ShIInFileItem(3));
		$_ff->SetTablename('sh_i_file_folder');
		$_ff->SetDocIdName('ship_id');
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['ship_id']);
		...
		$log->PutEntry($result['id'],'создал папку',NULL,648,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',649)){
		...
		$_ff->Edit($id, $params,NULL,$result,649);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,649,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['ship_id']), new ShIInFileItem(3), new FileDocFolderItem(3,abs((int)$_POST['ship_id']), new ShIInFileItem(3)));   //FileFolderList($storage_id);
	$_ff->setTableName('sh_i_file_folder');
