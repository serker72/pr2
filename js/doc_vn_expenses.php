...
if($_GET['term']) 	{
	...
	count(ch.id) as c_h 
	from doc_vn_expenses_kinds as p left join doc_vn_expenses_kinds as r on p.parent_id=r.id 
	left join doc_vn_expenses_kinds as q on r.parent_id=q.id 
	
	left join doc_vn_expenses_kinds as ch on p.id=ch.parent_id
