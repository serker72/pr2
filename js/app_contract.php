...
elseif(isset($_POST['action'])&&($_POST['action']=="add_comment")){
	...
	foreach($files_server as $k=>$file_server){
		...
		$log->PutEntry($result['id'],'прикреплен файл к комментарию  к лиду', NULL,950,NULL, ' омментарий '.$params['txt'].',  файл '.SecStr(iconv("utf-8","windows-1251",$files_client[$k])),$id);
