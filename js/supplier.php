...
elseif(isset($_POST['action'])&&($_POST['action']=="redraw_contact")){
	...
	$sm->assign('can_cont_edit', $au->user_rights->CheckAccess('w',165)&&($supplier['is_confirmed']!=1));
...
//	die();
	...
	$log=new ActionLog;
	$log->PutEntry((int)$result['id'],'добавил данные контакта контрагенту', NULL,165,NULL,SecStr(iconv("utf-8","windows-1251",$_POST['value'])), (int)$user_id);
	...
	$log=new ActionLog;
	$log->PutEntry((int)$result['id'],'редактировал данные контакта контрагента', NULL,165,NULL,SecStr(iconv("utf-8","windows-1251",$_POST['value']),9),(int)$user_id);
...
}elseif(isset($_POST['action'])&&($_POST['action']=="check_uncheck_customer")){
	//проверить, можно ли снять галочку Покупатель
		...
		$res=$_ki->CanUnCheckIsCustomer($id,$rss55);
		if($res!=="") $ret=$rss55;
		else $ret=0;
