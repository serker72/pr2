...
require_once('../classes/message_view.class.php');
...
elseif($_GET['term']) 	{
	...
	where p.is_active=1 and (p.name_s like "%'.iconv("utf-8","windows-1251",SecStr($_GET['term'])).'%" or p.login like "%'.iconv("utf-8","windows-1251",SecStr($_GET['term'])).'%") and p.id<>"'.$result['id'].'" and p.group_id<>2 '.$flt.' order by p.name_s asc ';
	//echo $sql;
