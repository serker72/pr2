...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,725,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',836)){
		...
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['user_d_id']), new PetitionFileItem(1));
		$_ff->SetTablename('petition_file_folder');
		$_ff->SetDocIdName('user_d_id');
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['user_d_id']);
		...
		$log->PutEntry($result['id'],'создал папку',NULL,836,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',836)){
		...
		$_ff=new FileDocFolderItem(1,abs((int)$_POST['user_d_id']), new PetitionFileItem);
		$_ff->SetTablename('petition_file_folder');
		$_ff->SetDocIdName('user_d_id');
		...
		$_ff->Edit($id, $params,NULL,$result,836);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,836,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['user_d_id']), new PetitionFileItem, new FileDocFolderItem(1,abs((int)$_POST['user_d_id']), new PetitionFileItem(1)));   //FileFolderList($storage_id);
	$_ff->setTableName('petition_file_folder');
