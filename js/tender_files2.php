...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу тендера описание',NULL,931,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	 if(!$au->user_rights->CheckAccess('w',931)){
		...
		$_ff=new FileDocFolderItem(2,abs((int)$_POST['id']), new tenderFileItem2(2));
	...
	//	$params['org_id']=$result['org_id'];
		...
		$log->PutEntry($result['id'],'создал папку',NULL,931,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$params['doc_id']);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	 if(!$au->user_rights->CheckAccess('w',931)){
		...
		$log->PutEntry($result['id'],'редактировал папку',NULL,931,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['bill_id']), new TenderFileItem2(2), new FileDocFolderItem(2,abs((int)$_POST['bill_id']), new TenderFileItem2(2)));   //FileFolderList($storage_id);
	$_ff->setTableName('tender_file_folder');
