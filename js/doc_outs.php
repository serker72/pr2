...
require_once('../classes/doc_out_history_fileitem.php');
require_once('../classes/doc_out_history_item.php');
require_once('../classes/doc_out_history_group.php');
...
require_once('../classes/doc_out_view.class.php');
...
elseif(isset($_POST['action'])&&(($_POST['action']=="retrieve_contacts")||($_POST['action']=="retrieve_only_contacts"))){
	$_sc=new DocOut_SupplierContactGroup;
	...
	if($_POST['action']=="retrieve_only_contacts") $ret=$sm->fetch('doc_out/suppliers_only_contacts.html');
	else $ret=$sm->fetch('doc_out/suppliers_contacts.html');
...
elseif(isset($_POST['action'])&&($_POST['action']=="check_confirm")){
		...
		$_res=new DocOut_Resolver($dem['kind_id']);
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_annul")){
	...
	$_ki=new DocOut_AbstractItem;
	
		$trust=$_ki->getitembyid($id);
		
	$_res=new DocOut_Resolver($trust['kind_id']);
	$_ti=$_res->instance;	
	...
	if(($trust['status_id']==18)&& ($trust['is_confirmed']==0)){
		...
		if($au->user_rights->CheckAccess('w',1065)){
			...
			$log->PutEntry($result['id'],'аннулирование исх. документа',NULL,1065,NULL,'исх. документ '.$trust['code'].': установлен статус '.$stat['name'],$id);	
			
			 
			 	 
			//внести примечание
			$_ni=new DocOut_HistoryItem;
		...
		if($au->user_rights->CheckAccess('w',1071)){
			$_ti->Edit($id,array('status_id'=>18, 'restore_pdate'=>time()), false,$result);
			
			$stat=$_stat->GetItemById(18);
			$log->PutEntry($result['id'],'восстановление исх. документа',NULL,1071,NULL,'исх. документ № '.$trust['code'].': установлен статус '.$stat['name'],$id);
	...
	$template='doc_out/table'.$prefix.'.html';
	...
	$ret= $acg->ShowPos(
		...
		$editing_user['can_annul']=$_res->instance->DocCanAnnul($editing_user['id'],$reason, NULL, $result)&&$au->user_rights->CheckAccess('w',1065);
		...
		$editing_user['can_restore']=$_res->instance->DocCanRestore($editing_user['id'],$reason)&&$au->user_rights->CheckAccess('w',1071);
		...
		$ret=$sm->fetch('doc_out/toggle_annul_card.html');		
	}
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_confirm_price")){
	...
	if($trust['is_confirmed']==1){
		...
		if($au->user_rights->CheckAccess('w',1072)&&$_ti->DocCanUnconfirmPrice($id, $rss)){
			    
				$_ti->Edit($id,array('is_confirmed'=>0, 'user_confirm_id'=>$result['id'], 'confirm_pdate'=>time()), true,$result);
				
				$log->PutEntry($result['id'],'снял утверждение заполнения исх. документа',NULL,1072, NULL, NULL,$id);
		...
		//есть права
		if($au->user_rights->CheckAccess('w',1065)&&$_ti->DocCanConfirmPrice($id, $rss)){
			 
				$_ti->Edit($id,array('is_confirmed'=>1, 'user_confirm_id'=>$result['id'], 'confirm_pdate'=>time()),true,$result);
				
				$log->PutEntry($result['id'],'утвердил заполнения исх. документа',NULL,1065, NULL, NULL,$id);	
...
elseif(isset($_POST['action'])&&(($_POST['action']=="add_supplier")||($_POST['action']=="add_supplier_15"))){
	...
	if((is_array($_POST['already_loaded'])&&!in_array( $_POST['supplier_id'], $_POST['already_loaded']))||!is_array($_POST['already_loaded'])){
		...
		if($_POST['action']=="add_supplier_15") $ret=$sm->fetch('doc_out/suppliers_15_table.html');
		else $ret=$sm->fetch('doc_out/suppliers_many_table.html');
	}
}
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_users")){
	...
	$ret.=$sm->fetch("doc_out/managers_set.html");
...
elseif(isset($_POST['action'])&&($_POST['action']=="add_comment")){
	...
	$_hi=new DocOut_HistoryItem; $_hg=new DocOut_HistoryGroup; $_dsi=new DocStatusItem; 
	$_file=new DocOut_HistoryFileItem;
	
	$_sch=new DocOut_Item;
	...
	$log->PutEntry($result['id'],'добавлен комментарий к исходящему документу', NULL,1065,NULL, $params['txt'],$id);
	...
	foreach($files_server as $k=>$file_server){
		...
		$log->PutEntry($result['id'],'прикреплен файл к комментарию  к исходящему документу', NULL,1065,NULL, 'Комментарий '.$params['txt'].',  файл '.SecStr(iconv("utf-8","windows-1251",$files_client[$k])),$id);
	...
	$_hr=new DocOut_HistoryGroup;
	
	$dec=new DBDecorator();
	$dec->AddEntry(new SqlEntry('o.id',$code, SqlEntry::E));
	
	$ret=$_hr->ShowHistory($id, 'doc_out/lenta.html', $dec, true, false, true,  $result,
			 $au->user_rights->CheckAccess('w',1066),
			 $au->user_rights->CheckAccess('w',1067));
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_comment")){
	...
	$log->PutEntry($result['id'],'редактирован комментарий к исходящему документу', NULL,1065,NULL, $params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="toggle_comment")){
	...
	if($hi['is_shown']==1){
		...
		$log->PutEntry($result['id'],'скрыт комментарий к исходящему документу', NULL,1065,NULL, 'Комментарий '.$hi['txt'].' ',$id);
		$ret=0;
	}else{
		$_hi->Edit($comment_id, array('is_shown'=>1));
		$log->PutEntry($result['id'],'показан комментарий к исходящему документу', NULL,1065,NULL, 'Комментарий '.$hi['txt'].' ',$id);
		$ret=1;
	}
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_pdf_addresses")){
	...
	$sm->assign('items', $alls);
	$ret=$sm->fetch('doc_out/pdf_addresses.html');
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_pdf_filelist")){
	...
	$sm->assign('items', $alls);
	$ret=$sm->fetch('doc_out/pdf_files.html');
...
//есть ли файлы по записи план-ка?
	...
	$sql='select count(*) from doc_out_file where bill_id="'.$id.'" ';
...
elseif(isset($_POST['action'])&&(($_POST['action']=="mode_reestr"))){
	$_views=new DocOut_ViewsGroup1;
	$_view=new DocOut_ViewsItem1;
...
//подгрузка пол-лей для согласователей/подписантов
elseif(isset($_POST['action'])&&($_POST['action']=="load_signs")){
	
	$kind=abs((int)$_POST['kind']); 
	
	$id=abs((int)$_POST['id']);
	$_bi1=new DocOut_AbstractItem;
	$bi1=$_bi1->GetItemById($id);
	...
	$sm->assign('kind',$kind);
	$sm->assign('can_modify',true);
	$sm->assign('can_confirm',false);
	$sm->assign('can_unconfirm',false);
	
 	if($kind==1) $sm->assign('many',true);
	else $sm->assign('many',false);
	
	$ret.=$sm->fetch("doc_out/sign_edit_set.html");
...
}elseif(isset($_POST['action'])&&($_POST['action']=="transfer_signs")){
	...
	$ret=$sm->fetch("doc_out/sign_many_table.html");
