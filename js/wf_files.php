...
require_once('../classes/wffileitem.php');
...
if(!$au->user_rights->CheckAccess('w',261)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
if(isset($_POST['action'])&&($_POST['action']=="edit_txt")){
	...
	$log->PutEntry($result['id'],'дал файлу описание',NULL,261,NULL,$txt);
...
elseif(isset($_POST['action'])&&($_POST['action']=="make_folder")){
	if(!$au->user_rights->CheckAccess('w',260)){
		header("HTTP/1.1 403 Forbidden");
		header("Status: 403 Forbidden");
		include("403.php");
		die();	
	}else{
		$_ff=new FileDocFolderItem(4,abs((int)$_POST['wf_id']), new WfFileItem(4));
		$_ff->SetTablename('interstore_file_folder');
		$_ff->SetDocIdName('wf_id');
		
		$params=array();
		$params['doc_id']=abs((int)$_POST['wf_id']);
		...
		$log->PutEntry($result['id'],'создал папку',NULL,260,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$code);
...
elseif(isset($_POST['action'])&&($_POST['action']=="edit_folder")){
	if(!$au->user_rights->CheckAccess('w',261)){
		...
		$_ff->Edit($id, $params,NULL,$result,261);
		
		$log->PutEntry($result['id'],'редактировал папку',NULL,261,NULL,'папка '.$params['filename'].', описание: '.$params['txt'],$id);
...
elseif(isset($_POST['action'])&&($_POST['action']=="load_folders")){
	...
	$_ff=new FileDocFolderList($storage_id, abs((int)$_POST['wf_id']), new WfFileItem(4), new FileDocFolderItem(4,abs((int)$_POST['wf_id']), new WfFileItem(4)));   //FileFolderList($storage_id);
	$_ff->setTableName('interstore_file_folder');
