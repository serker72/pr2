...
elseif(isset($_POST['action'])&&($_POST['action']=="redraw_deps_dics")){
	...
	*/
	...
	if($pi!==false) $value.=', поставщик '.$pi['name'];
	if($gi!==false) $value.=', раздел '.$gi['name'];
	$value=SecStr($value);
	
	
	$log->PutEntry($result['id'],'добавил индивидуальную максимальную скидку сотрудника', $params['user_id'],11,NULL, $value);
	...
	$sm->assign('user_id', $user_id);
	$sm->assign('named', 'Правила макс. скидок');
	...
	$log->PutEntry($result['id'],'удалил индивидуальную максимальную скидку сотрудника', $params['user_id'],11,NULL, $value);
