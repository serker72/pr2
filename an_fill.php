...
$log->PutEntry($result['id'],'перешел в Отчеты Заполненность данными',NULL, 599,NULL,NULL);
...
*/
	...
	if($au->FltSupplier($result)){  
		...
		$decorator->AddEntry(new SqlEntry('s.id', NULL, SqlEntry::IN_VALUES, NULL,$limited_supplier));
	}
...
//	$input_params[]=array('kind'=>'begin', 'label'=>'Справочная информация'); //Открываем группу данных
	...
	$input_params[]=array('kind'=>'begin', 'label'=>'Коммерческое предложение');  //Открываем группу данных
	...
	if($au->FltUser($result)){
		...
		$input_params[]=new AnFilesItem('Файлы коммерческого предложения', $id_count, new KPFileItem, 'kp_file.html', 695, false, NULL,  NULL, 'kp_id',   'kp', 'ed_kp.php', AnFilesItem::BY_PARENT_DOC,  'kp_id',  array('code'), 'КП № ', new KPItem(), 'kp_file_folder', $decorator); 
