...
unset($smarty);
	...
	if(isset($from)){
		...
		foreach($load_arr as $k=>$v){
			...
			$test_si=$_si->GetItemByFields(array('external_id'=>$v['BF'], 'org_id'=>(int)$result['org_id']));
			...
			if($test_si!==false){
				...
				if($opf_detected){
					...
					if($test_si['opf_id']=="0") $params['opf_id']=$detected_opf['id'];	
					
				} 
				 
				if($test_si['full_name']=="") $params['full_name']=SecStr($v['S']);
				$params['ur_or_fiz']=0;
				...
				if(($v['BA']!==NULL)&&($v['BA']!="")){
					...
					if($bi!==false){
						...
						if($test_si['branch_id']==0){
							if($bi['parent_id']!=0){
								//это подотрасль
								$params['subbranch_id']=$bi['id'];	
								$params['branch_id']=$bi['parent_id'];	
				...
				$supplier_id=$test_si['id'];
				$_si->Edit($test_si['id'], $params);
				
			}else{
				//нет такого поставщика	
			...
			if($supplier_id>0){
				...
				if(($v['B']!==NULL)&&($v['B']!="")){
					...
					if($city!==false){
						...
						//город нужно заносить, если такого города у поставщика нет!
						$test_city=$_sc->GetItemByFields(array('supplier_id'=>$supplier_id, 'city_id'=>$city['id']));
				...
				if(
				 ...
				 (($v['C']!==NULL)&&($v['C']!=""))
					...
					//заносим, если такого фактического адреса нет!
					$test_fa=$_fa->GetItemByFields(array('user_id'=>$supplier_id,
						'address'=>SecStr($note)));
