...
exec($comand);
 ...
 "; 
		...
		if($print_mode==0){
			...
			$log->PutEntry($result['id'],'получил pdf-версию БДР',NULL,1041, NULL, 'БДР № '.$editing_user['code'],$id);
			 
			header('Content-type: application/pdf');
			header('Content-Disposition: attachment; filename="БДР_'.$editing_user['code'].'.pdf'.'"');
			readfile(ABSPATH.'/tmp/'.$tmp.'.pdf');
		}elseif($print_mode==1){
			$log->PutEntry($result['id'],'получил pdf-версию БДДС',NULL,1041, NULL, 'БДР № '.$editing_user['code'],$id);
			 
			header('Content-type: application/pdf');
			header('Content-Disposition: attachment; filename="БДДС_'.$editing_user['code'].'.pdf'.'"');
			readfile(ABSPATH.'/tmp/'.$tmp.'.pdf');
		}
	}
