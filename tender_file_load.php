...
$_user=new Tender_AbstractItem;
$user=$_user->GetItemById($fmi['sched_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(1, $fmi['sched_id'], new TenderFileItem(1));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл тендера',NULL, 931, NULL,SecStr('файл '.$fmi['orig_name'].' для тендера '.  $user['code'].' '.$_folder_desrc),$user['id']);
