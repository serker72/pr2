...
$log=new ActionLog;
if($print==0){
	$log->PutEntry($result['id'],'открыл раздел Договоры и приложения',NULL,820);
}else{
	$log->PutEntry($result['id'],'открыл раздел Договоры и приложения: версия для печати',NULL,823);
}
...
/*********** OPO ****************************************************************************/		
	...
	if($print==0) $llg=$log->ShowPos('plan_fact_sales/plan_fact_sales_opo_list'.$print_add.'.html', 
	  $decorator, 
	  $from, 
	  $to_page,   
	...
	else  $llg=$log->ShowPos('plan_fact_sales/plan_fact_sales_opo_list'.$print_add.'.html', 
	  $decorator, 
	  0, 
	  1000000,   
