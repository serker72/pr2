...
require_once('classes/an_sched_funnel.php');
...
//******************************************************************************************************************	 
	...
	if(count($kinds)>0){
		...
		if($of_zero){
			//ничего нет - выбираем ВСЕ!	
			$decorator->AddEntry(new UriEntry('all_kinds',1));
			//вставка для исключения тендеров и лидов из общего отчета
			// $decorator->AddEntry(new SqlHavingEntry('`document_type_id`', NULL, SqlHavingEntry::IN_VALUES, NULL,array(1,2,3,4,5)));	
		}else{
	...
	if(isset($_GET['fo'.$prefix])&&(strlen($_GET['fo'.$prefix])>0)){
		...
		//$decorator->AddEntry(new SqlEntry('kind_id',7,SqlEntry::E));
	...
	$_tlist=new Tender_Group;
	$viewed_ids_arr[6]=$_tlist->GetAvailableTenderIds($result['id']);
	$_llist=new Lead_Group;
	$viewed_ids_arr[7]=$_llist->GetAvailableLeadIds($result['id']);
	
	
	//var_dump($viewed_ids_arr[5]);
	
	$filetext=$as1->ShowData($_suppliers, $_users,  $viewed_ids_arr,  $pdate1, $pdate2, 'an_sched/an_sched'.$prefix.$print_add.'.html',$decorator,'an_sched.php',   isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x'])/*||($print==1)*/,  $au->user_rights->CheckAccess('w',903),  $au->user_rights->CheckAccess('w',905), $alls, $result, $supplier_kinds, $sortmode, $is_fulfil);
...
//******************************************************************************************************************	 
	...
	/*
	if(isset($_GET['country'.$prefix])&&(strlen($_GET['country'.$prefix])>0)){
		if($print==1) $crit=SecStr(iconv("utf-8","windows-1251",$_GET['country'.$prefix]));
		else $crit=SecStr(($_GET['country'.$prefix]));
...
//******************************************************************************************************************	 
	...
	if(isset($_GET['country'.$prefix])&&(strlen($_GET['country'.$prefix])>0)){
		...
		$decorator1->AddEntry(new SqlEntry('p.id','select distinct sr.supplier_id from  supplier_sprav_city as sr inner join sprav_city as u on u.id=sr.city_id inner join sprav_country as sc on sc.id=u.country_id where sc.name LIKE "%'.$crit.'%" ', SqlEntry::IN_SQL));
		
		 
		
		$decorator3->AddEntry(new UriEntry('country',$crit));
	}	
	   */
...
//******************************************************************************************************************	 
	...
	//получим дату сравнения
	$other_date=NULL;
	if(isset($_GET['do_other_date'.$prefix])){
		$decorator->AddEntry(new UriEntry('do_other_date',1));
		
		$other_date=$_GET['pdate'.$prefix];
		$decorator->AddEntry(new UriEntry('pdate',$other_date));
	...
	if(isset($_GET['user'.$prefix])&&(strlen($_GET['user'.$prefix])>0)){
		...
		$decorator->AddEntry(new SqlEntry('u.id', NULL, SqlEntry::IN_VALUES, NULL,$_users));	
		$decorator->AddEntry(new UriEntry('user',  $_GET['user'.$prefix]));
	...
	*/
	include('inc/an_sched_supplier_simply_inc.php');
	...
	if(isset($_GET['country'.$prefix])&&(strlen($_GET['country'.$prefix])>0)){
		...
		$decorator->AddEntry(new SqlEntry('p.id','select distinct sr.supplier_id from  supplier_sprav_city as sr inner join sprav_city as u on u.id=sr.city_id inner join sprav_country as sc on sc.id=u.country_id where sc.id in ('.implode(', ',$_users1).') ', SqlEntry::IN_SQL));
	...
	if(isset($_GET['fo'.$prefix])&&(strlen($_GET['fo'.$prefix])>0)){
		...
		$decorator->AddEntry(new SqlEntry('p.id','select distinct sr.supplier_id from  supplier_sprav_city as sr inner join sprav_city as u on u.id=sr.city_id inner join sprav_district as sc on sc.id=u.district_id where sc.id in ('.implode(', ',$_users1).') ', SqlEntry::IN_SQL));
	...
	if(isset($_GET['branch'.$prefix])&&(strlen($_GET['branch'.$prefix])>0)){
	 ...
	 //расширенная форма
	 if(isset($_GET['view_reasons'.$prefix])&&($_GET['view_reasons'.$prefix]==1)){
	 	$decorator->AddEntry(new UriEntry('view_reasons', 1));
		 
	 
	 }else $decorator->AddEntry(new UriEntry('view_reasons', 0));
	...
	$as9=new AnSchedForgotten;
	
	$filetext=$as9->ShowData(/*'06.06.2015'*/$other_date,  'an_sched/an_sched'.$prefix.$print_add.'.html', $decorator, 'an_sched.php',isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x']), 
	$au->user_rights->CheckAccess('w',985),
	$au->user_rights->CheckAccess('w',87),
	$au->user_rights->CheckAccess('w',904),
	
	$alls,
	$result, true
//	$au->user_rights->CheckAccess('w',990)
	);
	...
	$sm->assign('has_9',  $au->user_rights->CheckAccess('w',984)||$au->user_rights->CheckAccess('w',985));

	 //фиксировать открытие отчета
	if( isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x'])/*||($print==1)*/){
		if($print==1) $log->PutEntry($result['id'],'открыл отчет Планировщик:Забытые клиенты',NULL,985,NULL, 'открыта версия для печати');	
		else $log->PutEntry($result['id'],'открыл отчет Планировщик:Забытые клиенты',NULL,984,NULL, NULL);	
	}
...
//******************************************************************************************************************	 
	...
	if(!isset($_GET['pdate_2'.$prefix])){
			...
			$_pdate2=DateFromdmY(date("d.m.Y"))+60*60*24-1;
			$pdate2=date("d.m.Y", $_pdate2);//"01.01.2006";	
	}else 
	{
		$pdate2 = $_GET['pdate_2'.$prefix];
		$_pdate2=DateFromdmY($pdate2)+60*60*24-1;
	}
	...
	//$decorator->AddEntry(new SqlOrdEntry('4',SqlOrdEntry::ASC)); 
	/*echo date('d.m.Y H:i:s', $_pdate1);
	echo date('d.m.Y H:i:s', $_pdate2);
	*/
	
	
	$as10=new AnSchedFunnel;
	
	$filetext10=$as10->ShowData( $_pdate1, $_pdate2, $_users, NULL,  'an_sched/an_sched'.$prefix.$print_add.'.html', $decorator, 'an_sched.php',isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x']), 
	$au->user_rights->CheckAccess('w',1001),
	...
	$sm->assign('log'.$prefix,$filetext10);
	
	$sm->assign('has_10',  $au->user_rights->CheckAccess('w',1000)||$au->user_rights->CheckAccess('w',1001));

	 //фиксировать открытие отчета
	if( isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x'])/*||($print==1)*/){
		if($print==1) $log->PutEntry($result['id'],'открыл отчет Планировщик:Воронка продаж',NULL,1001,NULL, 'открыта версия для печати');	
		else $log->PutEntry($result['id'],'открыл отчет Планировщик:Воронка продаж',NULL,1000,NULL, NULL);	
	}
	...
	/*elseif(isset($_GET['doSub7'])||isset($_GET['doSub7_x'])){
		...
		if(isset($_GET['doSub10'])||isset($_GET['doSub10_x'])){
			$cm='wkhtmltopdf-vor';
		}else{
			$cm='wkhtmltopdf-i386';
		}
		
		$comand = $cm."  --load-error-handling skip --encoding windows-1251 --page-size A4 ".$orient." --margin-bottom 27mm --margin-left 10mm --margin-right 10mm --footer-html ".ABSPATH."/tmp/".$ftmp.".html ".ABSPATH.'/tmp/'.$tmp.'.html'."  ".ABSPATH.'/tmp/'."$tmp.pdf 1>".ABSPATH.'/tmp/'."data 2> ".ABSPATH.'/tmp/'."exit";
