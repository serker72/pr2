...
if(isset($_GET['action'])&&($_GET['action']==2)){
	...
	if($file!==false){
		...
		$log->PutEntry($result['id'],'удалил учредительный документ',NULL,87,NULL,'имя файла '.SecStr($file['orig_name']),$sup_id);
	}
...
if(isset($_GET['action'])&&($_GET['action']==3)){
	...
	if($file!==false){
		$_ff->Del($id,$file,$result,87);
		
		$log->PutEntry($result['id'],'удалил папку',NULL,87,NULL,'имя папки '.SecStr($file['filename']));
		//echo 'zzz';
	}
...
if(isset($_GET['action'])&&($_GET['action']==4)){
	...
	$_ff->SetDocIdName('sup_id');
		    ...
		    $_ff->Edit($v, array('parent_id'=>$move_folder_id), NULL,$result, 87);
