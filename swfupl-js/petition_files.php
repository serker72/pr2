<?php
	...
	if (isset($_POST["user_d_id"])){
		$petition_id=abs((int)$_POST["user_d_id"]);	
	}else{
	...
	if(isset($_FILES['Filedata'])){
		...
		$code=$_fi->Add(array('user_d_id'=>$petition_id,'user_id'=>$result['id'], 'orig_name'=>SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),'folder_id'=>abs((int)$_POST['folder_id']), 'filename'=>basename($tempname), 'pdate'=>time()));
		
		$log->PutEntry( $result['id'],'добавил файл заявления',NULL,836,NULL,'имя файла '.SecStr(iconv("utf-8","windows-1251", $_FILES['Filedata']['name'])),$petition_id);
