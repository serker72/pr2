...
if(isset($_FILES['file']['tmp_name'])){
	...
	header("Location: import_russian_prodim.php?file=".SecureCyr($_FILES['file']['name'])."&from=1");
	die();
}
...
unset($smarty);
    ...
    <?
		...
		foreach($aSheet->getRowIterator() as $row){
			 ...
			 // var_dump($values);
			  ...
			  if(($values[1]!="")&&($values[2]!="")){
					echo 'found data: '.$values[1].' and '.$values[2].'  <br>';
		...
		if(($from+$items_per)>$index){
			...
			// не конец
			?>
			<a href="import_russian_prodim.php?file=<?=$file?>&from=<?=($from+$items_per)?>">продолжить работу</a>
            <script type="text/javascript">
			function Away(){
				location.href='import_russian_prodim.php?file=<?=$file?>&from=<?=($from+$items_per)?>';
