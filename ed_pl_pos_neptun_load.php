		...
		$aSheet = $objPHPExcel->getActiveSheet();
		 ...
		 foreach($aSheet->getRowIterator() as $row){
			  ...
			  /*if(($values[0]!='')&&($values[1]=='')&&($values[2]=='')&&($values[3]=='')){
				  ...
				  echo '<strong>Это позиция, ее код='.$values[0].'</strong><br>';
				 // echo '<strong>в группу опций '. $last_group.'</strong><br>';
				  ...
				  if($values[0]==''){
					  $test_pos=$ui->GetItemByFields(array('name'=>SecStr($values[5],9), 'parent_id'=>$parent_id));
				 
				  		$translate_pos=$ui->GetItemByFields(array('name_en'=>SecStr($values[5],9)));
				  }else{
				  
				  
				  	$test_pos=$ui->GetItemByFields(array('code'=>SecStr($values[0]), 'parent_id'=>$parent_id));
				 
				  	$translate_pos=$ui->GetItemByFields(array('code'=>SecStr($values[0]), 'parent_id'=>$parent_id));
				  
				  }
				 // var_dump($translate_pos);
				  $pos_id=0;
				  if($test_pos===false){
						echo '<span style="color:red;">Позиция '. $values[0].' НЕ найдена, создаем... </span><br>'; 
						
						$params=array();
						$params['code']=SecStr($values[0]);
						//$params['group_id']=abs((int)$_POST['group_id']);
						
						$params['group_id']=$editing_user['group_id'];
						$params['parent_id']=$editing_user['id'];
						
						if(($translate_pos==false)) $params['name']=SecStr($values[2],10);
						else{
						...
						$params['pl_group_id']=28;//$pl_group_id;
						 
						//print_r($params); 
						 						
						$pos_id=$ui->Add($params);
				  }else{
						echo 'Позиция '. $values[0].'   найдена, ок... <br>';    
						//$pl_group_id=$test_plgg['id'];
						$pos_id=$test_pos['id'];
				  ...
				  $pl_id=0;
				  if($test_pl===false){
						echo '<span style="color:red;">Позиция ПЛ '. $values[0].' НЕ найдена, создаем... </span><br>';  						
						...
						echo 'Позиция ПЛ '. $values[0].'    найдена, меняем ей цену ок... <br>';    
						//$pl_group_id=$test_plgg['id'];
						$pl_id=$test_pl['id'];
					...
					$price_params=array();
					$price_params['price']=abs((float)str_replace(",",".",$values[5]));
					...
					if($test_price===false){
						//цены нет, внести ее	
						echo '<span style="color:red;">ЦЕНА '. $values[5].' НЕ найдена, создаем... </span><br>'; 
						...
						echo 'ЦЕНА '. $values[5].'   найдена и равна '.$test_price['price'].', меняем ее... <br>';
