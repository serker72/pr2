...
/*	
	...
	$positions=array();
	 ...
	 //исключение - если сотрудник=координатор, помощник - то не добавлять
	$_user_pos=new UserPosItem;
	$user_pos=$_user_pos->GetItemById($result['position_id']);
	
	if(!eregi('координатор', $user_pos['name'])&&!eregi('помощник', $user_pos['name'])){
	...
	$_resp_users=$ui->GetUsersForResp($id, $editing_user, $for_resp_ids);
	 
	if($au->user_rights->CheckAccess('w',910)||in_array($result['id'], $for_resp_ids)){
		$_user=new UserSItem;
		$positions=array();
	...
	$log->PutEntry($result['id'],'удалил контрагента',NULL,88, NULL, SecStr($editing_user['full_name']),$id);	
...
$_menu_id=27;
	...
	if($action==0){
		...
		$sm1->assign('resp_users', $_rg->GetUsersArr($id));		
			...
			$_resp_users=$ui->GetUsersForResp($id, $editing_user, $for_resp_ids);
		$sm1->assign('can_edit_resp',  $au->user_rights->CheckAccess('w',910)||in_array($result['id'], $for_resp_ids));
		
		
		//print_r($_resp_users);
		$sm1->assign('for_resp_users', implode(', ', $_resp_users));
...
1004
			...
			$llg=$log->ShowLog('syslog/log_doc.html',$decorator,$from,$to_page,'supplier.php',true,true,true);
...
//вкладка Действия по к-ту
	...
	switch($sortmode){
		...
		break;	
			...
			/*$viewed_ids=$_sched->GetAvailableUserIds($result['id']);
			$notes_viewed_ids=$_sched->GetNotesAvailableUserIds($result['id']);*/
	...
	$viewed_ids_arr[7]=$_llist->GetAvailableLeadIds($result['id']); 
			...
			$filetext=$as1->ShowData(array($id),  $_users,   $viewed_ids_arr,  $pdate1, $pdate2, 'suppliers/an_sched'.$prefix.'.html',$decorator,'supplier.php',   true,  $au->user_rights->CheckAccess('w',903),  $au->user_rights->CheckAccess('w',905), $alls, $result, NULL, $sortmode, $is_fulfil);
...
//	if(isset($_GET['to_page'.$prefix])) $to_page=abs((int)$_GET['to_page'.$prefix]);
	...
	if($cou_stat>0){
	  ...
	  $cou_stat=0; foreach($_COOKIE as $k=>$v) if(eregi('^'.$prefix.'tools_'.$prefix.'status_id_', $k)) $cou_stat++;
	  
	  if($cou_stat>0){
		  //есть кукисы
		  foreach($_COOKIE as $k=>$v) if(eregi('^'.$prefix.'tools_'.$prefix.'status_id_', $k)) $status_ids[]=(int)eregi_replace('^'.$prefix.'tools_'.$prefix.'status_id_','',$k);
	  }else{
		  //ничего нет - выбираем ВСЕ!	
		  $decorator->AddEntry(new UriEntry('all_statuses',1));
	  }
   ...
   if(count($status_ids)>0){
	...
	$decorator->AddEntry(new SqlEntry('p.supplier_id', $id, SqlEntry::E));	
 
	
	$_tools=new SupplierTool_Resolver(4);
	$_tools->SetAuthResult($result);
	
	$tools=$_tools->group_instance->ShowPos(  
		  'suppliers/supplier_tools.html',  //0
		  $decorator, //1
	...
	$sm->assign('tools', $tools);
	$sm->assign('has_tools_tab', ($editing_user['is_customer']==1));
	...
	if((isset($_GET['from_begin'])&&($_GET['from_begin']==1))||($action==0)) $sm->assign('from_begin', 1);
