...
$_user=new PrikazItem;
$user=$_user->GetItemById($fmi['prikaz_id']);

if($fmi['folder_id']==0){
	$_folder_desrc='';
}else{
	$_ff=new FileDocFolderItem(4, $fmi['prikaz_id'], new PrikazFileItem(4));
...
$log=new ActionLog;
$log->PutEntry($result['id'],'скачал файл приказа',NULL,805, NULL,SecStr('файл '.$fmi['orig_name'].' для '.  $user['id'].' вход. № '.$user['vhod_no'].$_folder_desrc),$user['id']);
