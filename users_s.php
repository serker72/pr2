...
//ограничени€ по сотруднику
	...
	if(isset($_GET['position_s'])&&(strlen($_GET['position_s'])>0)){
		$decorator->AddEntry(new SqlEntry('pos.name',SecStr($_GET['position_s']), SqlEntry::LIKE));
		$decorator->AddEntry(new UriEntry('position_s',$_GET['position_s']));
	}
	
	if(isset($_GET['department_s'])&&(strlen($_GET['department_s'])>0)){
		$decorator->AddEntry(new SqlEntry('dep.name',SecStr($_GET['department_s']), SqlEntry::LIKE));
		$decorator->AddEntry(new UriEntry('department_s',$_GET['department_s']));
	}
	
	if(isset($_GET['main_department_s'])&&(strlen($_GET['main_department_s'])>0)){
		$decorator->AddEntry(new SqlEntry('mdep.name',SecStr($_GET['main_department_s']), SqlEntry::LIKE));
		$decorator->AddEntry(new UriEntry('main_department_s',$_GET['main_department_s']));
	}
	...
	switch($sortmode){
		...
		break;	
		case 6:
			$decorator->AddEntry(new SqlOrdEntry('position_name',SqlOrdEntry::DESC));
		break;
		case 7:
			$decorator->AddEntry(new SqlOrdEntry('position_name',SqlOrdEntry::ASC));
		break;
		case 8:
			$decorator->AddEntry(new SqlOrdEntry('department_name',SqlOrdEntry::DESC));
		break;
		case 9:
			$decorator->AddEntry(new SqlOrdEntry('department_name',SqlOrdEntry::ASC));
		break;
		
		case 10:
			$decorator->AddEntry(new SqlOrdEntry('main_department_name',SqlOrdEntry::DESC));
		break;
		case 11:
			$decorator->AddEntry(new SqlOrdEntry('main_department_name',SqlOrdEntry::ASC));
		break;
	...
	if($print==0) $uug= $ug->GetItems('users/s'.$print_add.'.html',$decorator,$from,$to_page,$in_storage,$in_sector,  $au->user_rights->CheckAccess('w',512), $au->user_rights->CheckAccess('w',542),'',1, $au->user_rights->CheckAccess('w',805), $limited_user);
	else $uug= $ug->GetItems('users/s'.$print_add.'.html',$decorator,0,1000000,$in_storage,$in_sector,  $au->user_rights->CheckAccess('w',512),  $au->user_rights->CheckAccess('w',542), '',1,false,$limited_user );
...
//**************************** виртуальные сотрудники ****************************************************************/	
	...
	if(isset($_GET['position_s'.$prefix])&&(strlen($_GET['position_s'.$prefix])>0)){
		$decorator->AddEntry(new SqlEntry('pos.name',SecStr($_GET['position_s'.$prefix]), SqlEntry::LIKE));
		$decorator->AddEntry(new UriEntry('position_s',$_GET['position_s'.$prefix]));
	}
	
	if(isset($_GET['department_s'.$prefix])&&(strlen($_GET['department_s'.$prefix])>0)){
		$decorator->AddEntry(new SqlEntry('dep.name',SecStr($_GET['department_s'.$prefix]), SqlEntry::LIKE));
		$decorator->AddEntry(new UriEntry('department_s',$_GET['department_s'.$prefix]));
	}
	...
	$ug1=new UsersVGroup;
	if($print==0) $uug1= $ug1->GetItems('users/s'.$print_add.'.html',$decorator,$from,$to_page,$in_storage,$in_sector,  $au->user_rights->CheckAccess('w',780), $au->user_rights->CheckAccess('w',770),$prefix, 3, $limited_user);
	else $uug1= $ug1->GetItems('users/s'.$print_add.'.html',$decorator,0,1000000,$in_storage,$in_sector,  $au->user_rights->CheckAccess('w',780),  $au->user_rights->CheckAccess('w',770),$prefix, 3, $limited_user);
	...
	if((count($qsts)>0)&&(isset($_GET['doSub'])||isset($_GET['doSub.x'])||isset($_GET['doSub_x']))&&($print==0)){
