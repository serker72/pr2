...
if($print!=0){
	if(!$au->user_rights->CheckAccess('w',353)&&!$au->user_rights->CheckAccess('w',488)){
	  header("HTTP/1.1 403 Forbidden");
	  header("Status: 403 Forbidden");
	  include("403.php");
	  die();	
...
if(!$au->user_rights->CheckAccess('w',352)&&!$au->user_rights->CheckAccess('w',487)){
	header("HTTP/1.1 403 Forbidden");
	header("Status: 403 Forbidden");
	include("403.php");
	die();	
...
/*********************вкладка оригиналы отгруз докум - входящие********************************************/
	...
	if(isset($_GET['supplier_name'.$prefix])&&(strlen($_GET['supplier_name'.$prefix])>0)){
		$decorator->AddEntry(new SqlEntry('sp.full_name',NULL, SqlEntry::LIKE_SET, NULL, explode(';',SecStr($_GET['supplier_name'.$prefix]))));//SqlEntry::LIKE));
	...
	//$filetext=$as->ShowData(NULL,NULL,$mode, $result['org_id'],$decorator,'original/original'.$print_add.'.html','original.php', $au->user_rights->CheckAccess('w',353),isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x'])||($print==1));
	...
	$sm->assign('has_1',$au->user_rights->CheckAccess('w',352));
	

	//фиксировать открытие отчета
	if( isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x'])||($print==1)){
		$log->PutEntry($result['id'],'открыл отчет Оригиналы отгрузочных документов - Входящие',NULL,352,NULL, NULL);	
	}
...
/*********************вкладка оригиналы договоров и учредит докум********************************************/	
	...
	if(isset($_GET['supplier_name'.$prefix])&&(strlen($_GET['supplier_name'.$prefix])>0)){
		...
		$decorator->AddEntry(new SqlEntry('p.full_name',NULL, SqlEntry::LIKE_SET, NULL, explode(';',$supplier_name)));//SqlEntry::LIKE));
	...
	//$filetext=$as->ShowData($has_dog3, $has_uch3, $has_dog_in3, $has_no_dog3, $has_no_uch3, $has_no_dog_in3,  $decorator, 'original/original_dog'.$print_add.'.html','original.php', $au->user_rights->CheckAccess('w',488),  isset($_GET['doSub'.$prefix])||isset($_GET['doSub'.$prefix.'_x'])||($print==1));
